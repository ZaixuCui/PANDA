...
% Last Modified by GUIDE v2.5 05-May-2012 19:37:03
...
BrainParcellationPipeline_opt.mode = 'batch';
...
if ~isempty(QuantityOfCpu)
    BrainParcellationPipeline_opt.max_queued = str2num(QuantityOfCpu);
else
...
BrainParcellationPipeline_opt.path_logs = [pwd filesep 'logs'];
...
if ~isempty(FAPathCellBrainParcellation)
    [x,T1PathCellBrainParcellation] = Select('img');
...
if exist(LockFilePath, 'file') | strcmp(get(handles.FAPathButton, 'Enable'), 'off')
    ...
    elseif isempty(PartitionTemplatePath)
    ...
    if BrainParcellationInputFinish & ~LogPathPermissionDenied
        ...
        if strcmp(button, 'Yes')
            ...
            [ ParameterSaveFileName,ParameterSavePath ] = uiputfile( {'*.PANDA_BrainParcellation', 'BrainParcellation-files (*.PANDA_BrainParcellation)'},'Save Configuration' );
            ...
            if ParameterSavePath ~= 0
                cmdString = [ 'save ' ParameterSaveFilePath ' FAPathCellBrainParcellation' ' T1PathCellBrainParcellation' ' BrainParcellationPipeline_opt' ...
                    ' PartitionTemplatePath' ' PANDAPath'];
                eval(cmdString);
                ...
                command = ['"' matlabroot filesep 'bin' filesep 'matlab" -nosplash -nodesktop -r "load(''' ParameterSaveFilePath ''',''-mat''); addpath(genpath(PANDAPath));'...
                    'pipeline=g_BrainParcellation_pipeline( FAPathCellBrainParcellation, T1PathCellBrainParcellation, PartitionTemplatePath, BrainParcellationPipeline_opt );exit"'...
                ...
                system(['. ' BrainParcellationPipelineShLocation ' &']);
                fclose(fid);
                %g_BrainParcellation_pipeline(FAPathCellBrainParcellation, T1PathCellBrainParcellation, PartitionTemplatePath, BrainParcellationPipeline_opt);
                ...
                for i = 1:length(FAPathCellBrainParcellation)
                    ...
                    JobsStatus{i, 2} = 'wait';
                    JobsStatus{i, 3} = 'PartitionTemplate2FA';
                    JobsStatus{i, 4} = '1';
                end
...
% handles    structure with handles and user data (see GUIDATA)
global FAPathCellBrainParcellation;
[a, FAPathCellBrainParcellation] = Select('img');
if ~isempty(FAPathCellBrainParcellation)
...
switch button
    case 'Yes'
        ...
        clear global PartitionTemplatePath;
...
if exist( StatusFilePath, 'file' )
    ...
    try     
        ...
        StatusStruct = load(StatusFilePath);
        JobNames = fieldnames(StatusStruct);
        for i = 1:length(JobNames)
            SubjectIDArrayString{i} = JobNames{i}(end-4:end);
            if ~strcmp(StatusStruct.(JobNames{i}), 'finished')
                JobNameArray{i} = JobNames{i}(1:end-6);
            else
                JobNameArray{i} = '';
            end
            if ~strcmp(StatusStruct.(JobNames{i}), 'none')
                StatusArray{i} = StatusStruct.(JobNames{i});
            else
                StatusArray{i} = 'wait';
            end
            if strcmp(StatusArray{i}, 'none') | strcmp(StatusArray{i}, 'failed')
                JobLeftArray{i} = '1';
            else
                JobLeftArray{i} = '0';
            end
...
if strcmp(get(handles.FAPathButton, 'Enable'), 'off')
    ...
    if ParameterSaveFileName ~= 0
        ...
        eval( cmdString );
        set( handles.PartitionTemplateEdit, 'String', PartitionTemplatePath);
        if strcmp(BrainParcellationPipeline_opt.mode,'batch')
            set( handles.batchRadio, 'Value', 1 );
        else
            set( handles.batchRadio, 'Value', 1 );
        ...
        set( handles.MaxQueuedEdit, 'string', num2str(BrainParcellationPipeline_opt.max_queued) );
        set( handles.LogPathEdit, 'string', BrainParcellationPipeline_opt.path_logs);
...
BrainParcellationPipeline_opt.path_logs = [LogPath filesep 'logs'];
