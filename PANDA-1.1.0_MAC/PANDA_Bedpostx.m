...
% Last Modified by GUIDE v2.5 05-May-2012 19:46:36
...
BedpostxPipeline_opt.mode = 'batch';
...
global NativePathSelectedBedpostx;
NativePathSelectedBedpostx = get(hObject, 'Value');
...
global NativePathCellBedpostx;
global NativePathSelectedBedpostx;
NativePathCellBedpostx = get(handles.NativeFolderListbox, 'String');
if NativePathSelectedBedpostx 
    OldNativePathCell = NativePathCellBedpostx;
    NativePathCellBedpostx = '';
    j = 0;
    for i = 1:length(OldNativePathCell)
        if ~sum(ismember(NativePathSelectedBedpostx, i))
            j = j + 1;
            NativePathCellBedpostx{j} = OldNativePathCell{i};
        end
    end
    NativePathSelectedBedpostx = 0;
    NativePathCellBedpostx = reshape(NativePathCellBedpostx, length(NativePathCellBedpostx), 1);
...
NativePathCellBedpostx = [ NativePathCellBedpostx;AddNativePathCell ];
set(handles.NativeFolderListbox, 'Value', 1);
set(handles.NativeFolderListbox, 'String', NativePathCellBedpostx);
...
if exist(LockFilePath, 'file') | strcmp(get(handles.NativeFolderListbox, 'Enable'), 'off')
    ...
    if BedpostxInputFinish & ~LogPathPermissionDenied
        ...
        if strcmp(button, 'Yes')
            ...
            [ ParameterSaveFileName,ParameterSavePath ] = uiputfile( {'*.PANDA_Bedpostx', 'Bedpostx-files (*.PANDA_Bedpostx)'},'Save Configuration' );
...
BedpostxPipeline_opt.path_logs = [LogPath filesep 'logs'];
...
if strcmp(get(handles.NativeFolderListbox, 'Enable'), 'off')
    ...
    if ParameterSaveFileName ~= 0
        ...
        set( handles.NativeFolderListbox, 'Value', 1 );
        set( handles.NativeFolderListbox, 'String', NativePathCellBedpostx );
        if strcmp(BedpostxPipeline_opt.mode,'batch')
            set( handles.batchRadio, 'Value', 1 );
        else
        ...
        set( handles.MaxQueuedEdit, 'string', num2str(BedpostxPipeline_opt.max_queued) );
        set( handles.LogPathEdit, 'string', BedpostxPipeline_opt.path_logs);
...
if exist( StatusFilePath, 'file' )
    ...
    try     
        ...
        for i = 1:SubjectQuantity
            ...
            JobName{1} = [ 'BedpostX_preproc_',Number_Of_Subject_String ];
            BedpostXPreStatus = eval(JobName{1});
            for BedpostXJobNum = 1:10
                JobName{BedpostXJobNum + 1} = [ 'BedpostX_' Number_Of_Subject_String '_' num2str(BedpostXJobNum, '%02.0f') ];
                BedpostXJobStatus{BedpostXJobNum} = eval(JobName{BedpostXJobNum + 1});
            end
            JobName{12} = [ 'BedpostX_postproc_' Number_Of_Subject_String ];
            BedpostXPostStatus = eval(JobName{12});
            
            SubjectIDArrayString{i} = Number_Of_Subject_String;
            ...
            elseif strcmp(BedpostXPostStatus, 'finished')
                StatusArray{i} = ['finished' BedpostxStopFlag];
                JobLeftArray{i} = '0';
                JobNameArray{i} = '';
...
%             if ~isempty(BedpostxStopFlag)
%                 StatusArray{i} = [StatusArray{i} BedpostxStopFlag];
%             end
