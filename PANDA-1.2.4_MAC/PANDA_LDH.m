...
% Last Modified by GUIDE v2.5 06-Sep-2013 14:48:48
...
global LDH_DWI_Path_Cell;
global LDH_SubjectIDArray;
global LDH_DestinationPath_Edit;
global LDH_PrefixEdit;
...
LDH_Pipeline_opt.path_logs = [pwd filesep 'LDH_logs'];
    ...
    if ~isempty(SubjectID_Text)
        try
            ...
            if length(LDH_DWIList) ~= length(LDH_SubjectIDArray)
                ...
                ResultantFileList = cell(length(LDH_DWIList), 1);
                for i = 1:length(LDH_SubjectIDArray)
                    [ResultantFolder y z] = fileparts(LDH_DWIList{i});
                    ResultantFileList{i} = [ResultantFolder filesep num2str(LDH_SubjectIDArray(i), '%05d') '_xxLDHx_xxx.nii.gz'];  
                end
                RawData_Destination = [LDH_DWIList LDH_BvalsList ResultantFileList];
...
if isempty(LDH_SubjectIDArray)
    msgbox('Please input IDs for subjects.');
    set(hObject, 'String', '');
else
    ...
    % Add the prefix to resultant file 
    ResultantFileList = cell(length(LDH_DWIList), 1);
    for i = 1:length(LDH_SubjectIDArray)
        [ResultantFolder y z] = fileparts(LDH_DWIList{i});
        if ~isempty(LDH_FilePrefixEdit)
            ResultantFileList{i} = [ResultantFolder filesep LDH_FilePrefixEdit '_' num2str(LDH_SubjectIDArray(i), '%05d') '_xxLDHx_xxx.nii.gz'];
        else
            ResultantFileList{i} = [ResultantFolder filesep num2str(LDH_SubjectIDArray(i), '%05d') '_xxLDHx_xxx.nii.gz'];
        end
    end
...
LDHStopFlag = '';
LockFilePath = [LDH_Pipeline_opt.path_logs filesep 'PIPE.lock'];
if exist(LockFilePath, 'file') | strcmp(get(handles.DWIPathButton, 'Enable'), 'off')
    ...
    elseif isempty(LDH_SubjectIDArray)
        ...
        try
            if ~exist(LDH_Pipeline_opt.path_logs, 'dir')
                mkdir(LDH_Pipeline_opt.path_logs);
            end
            x = 1;
            save([LDH_Pipeline_opt.path_logs filesep 'permission_tag.mat'], 'x');
        catch
        ...
        if ~LogPathPermissionDenied
            ...
            if strcmp(button, 'Yes')
                ...
                ParameterSaveFilePath = [LDH_Pipeline_opt.path_logs  filesep DateNowString '.PANDA_LDH'];
                cmdString = [ 'save ' ParameterSaveFilePath ' LDH_DWIList' ' LDH_BvalsList' ' LDH_SubjectIDArray' ...
                ...
                if exist( ParameterSaveFilePath, 'file' )
                    % Excute the pipeline
                    if ~exist(LDH_Pipeline_opt.path_logs)
                        mkdir(LDH_Pipeline_opt.path_logs);
                    end
                    
                    LDH_Pipeline_opt.mode_pipeline_manager = 'batch';
                    LDH_Pipeline_opt.flag_verbose = 0;
                    LDH_Pipeline_opt.flag_pause = 0;
                    ...
                    for i = 1:length(LDH_DWIList)
                        ...
                        if ~isempty(LDH_FilePrefixEdit)
                            ...
                            FilePrefix = LDH_FilePrefixEdit;
                        end
                        
                        Job_Name = ['DWI2LDH_' Number_Of_Subject_String];
                        pipeline.(Job_Name).command           = 'g_DWI2LDH( opt.DWI4D,opt.Bval,opt.Nvoxel,opt.Prefix,opt.SubjectID,opt.type )';
                        pipeline.(Job_Name).files_out.files{1}= [ResultantFolder filesep  FilePrefix '_' num2str(Neighborhood - 1, '%02d') 'LDHs.nii.gz'];
                        pipeline.(Job_Name).files_out.files{2}= [ResultantFolder filesep  FilePrefix '_' num2str(Neighborhood, '%02d') 'LDHk.nii.gz'];
                    ...
                    StatusFilePath = [LDH_Pipeline_opt.path_logs filesep 'PIPE_status.mat'];
...
if strcmp(get(handles.DWIPathButton, 'Enable'), 'off')
    ...
    if ParameterSaveFileName ~= 0
        if ~isempty(LDH_Pipeline_opt.path_logs)
            LockFilePath = [LDH_Pipeline_opt.path_logs filesep 'PIPE.lock'];
        end
        ...
        set( handles.RawPath_DestPath, 'data', RawData_Destination);
        ResizeRawPathDestPathTable(handles);
        if strcmp(LDH_Pipeline_opt.mode,'batch')
            set( handles.batchRadio, 'Value', 1 );
        else
        ...
        set( handles.MaxQueuedEdit, 'string', num2str(LDH_Pipeline_opt.max_queued) );
        [LogParent, y, z] = fileparts(LDH_Pipeline_opt.path_logs);
...
if isempty(LDH_DWIList)
    ...
    if ParameterSaveFileName ~= 0
        ...
        eval( cmdString );
        [LogParentFolder y z] = fileparts(PANDAConfiguration.LDH_Pipeline_opt.path_logs);
        LogsPath = [LogParentFolder filesep 'logs'];    
    end
else
    try
        PANDALogsFile = [LDH_Pipeline_opt.path_logs filesep 'PIPE_logs.mat'];
        PANDAStatusFile = [LDH_Pipeline_opt.path_logs filesep 'PIPE_status.mat'];
        if exist(PANDALogsFile, 'file') & exist(PANDAStatusFile, 'file')
            ...
            if ~FailedQuantity
                ...
                PANDA_FailedLogs(PANDALogs, FailedJobNames, LDH_Pipeline_opt.path_logs);
            end
...
if isempty(LDH_DWIList)
    ...
    [a, LDH_BvalsList, Done] = PANDA_Select('file', BvalsList_Button);
    if Done == 1
...
WidthCell{1} = PositionFigure(3) / 3;
...
switch get(hObject, 'tag')
    ...
    case 'qsubRadio'
        ...
        LDH_Pipeline_opt.qsub_options = '-q all.q';
        set( handles.QsubOptionsEdit, 'Enable', 'on');
...
StatusFilePath = [LDH_Pipeline_opt.path_logs filesep 'PIPE_status_backup.mat'];
if exist( StatusFilePath, 'file' )
    ...
    try    
        ...
        % 
        JobsFinishFilePath = [LDH_Pipeline_opt.path_logs filesep 'jobs.finish'];
