...
ResamplePipeline_opt.mode = 'batch';
...
if ~isempty(QuantityOfCpu)
    ResamplePipeline_opt.max_queued = str2num(QuantityOfCpu);
else
    ...
    elseif isempty(VoxelSize)
        ...
        if ~LogPathPermissionDenied
            ...
            if strcmp(button, 'Yes')
                ...
                % Excute the pipeline
                ResamplePipeline_opt.mode_pipeline_manager = 'batch';
...
if strcmp(get(handles.RawImagesButton, 'Enable'), 'off')
    ...
    if ParameterSaveFileName ~= 0
        ...
        RawResampledImagesTable(handles);
        if strcmp(ResamplePipeline_opt.mode,'batch')
            set( handles.batchRadio, 'Value', 1 );
        else
...
switch get(hObject, 'tag')
    ...
    case 'qsubRadio'
        ...
        ResamplePipeline_opt.qsub_options = '-q all.q';
        set( handles.QsubOptionsEdit, 'Enable', 'on');
...
if ~isempty(RawPathDestPath)
    ...
    for i = 1:columns
        for j = 1:rows
            ...
            tmp{j} = tmp{j} * PositionFigure(4) / 638;
        end
