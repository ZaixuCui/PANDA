...
% Last Modified by GUIDE v2.5 03-May-2012 17:07:28
...
if isempty(FSLDIR)
    msgbox('please install FSL first !');
    delete(hObject);
end
...
[a, Data_Raw_Path_Cell] = Select('dir');
        ...
        if ~LogPathPermissionDenied
            ...
            switch button
                case 'Yes'
                    ...
                    [ ParameterSaveFileName,ParameterSavePath ] = uiputfile( {'*.PANDA', 'PANDA-files (*.PANDA)'},'Save Configuration' );
                    ...
                    if ParameterSavePath ~= 0
                        ...
                        system(['. ' DTIPipelineShLocation ' &']);
                        fclose(fid);
                        %                   pipeline = g_dti_pipeline( Data_Raw_Path_Cell, SubjectIDArray, DestinationPath_Edit, TensorPrefixEdit, pipeline_opt, dti_opt, tracking_opt, T1orPartitionOfSubjectsPathCellMain );
                        % Set edit box unable
...
%                         end
                        ...
                        command = ['"' matlabroot filesep 'bin' filesep 'matlab" -nosplash -nodesktop -r "load(''' ParameterSaveFilePath ''',''-mat'');'...
                                'addpath(genpath(PANDAPath));CalculateJobStatusMain(DestinationPath_Edit,SubjectIDArray,dti_opt,tracking_opt);exit"'...
                        ...
                        fprintf(fid, '%s', command);
                        system(['. ' CalculateJobStatusMainShLocation ' &']);
                        fclose(fid);
                        % Start monitor function
                        StopFlag = '';
                        JobStatusMonitorTimer = timer( 'TimerFcn', {@JobStatusMonitorMain, handles}, 'period', 10, 'ExecutionMode', 'fixedRate');
                        start(JobStatusMonitorTimer);
                    end
                case 'No'
                    return;
            end
                ...
                if length(Data_Raw_Path_Cell) ~= length(SubjectIDArray)
                    ...
                    if exist(LockFilePath, 'file')
                        ...
                        command = ['"' matlabroot filesep 'bin' filesep 'matlab" -nosplash -nodesktop -r "load(''' ParameterSaveFilePath filesep ParameterSaveFileName ...
                            ''',''-mat''); addpath(genpath(PANDAPath)); CalculateJobStatusMain(DestinationPath_Edit,SubjectIDArray,dti_opt,tracking_opt,1);exit"'...
                                ' >"' DestinationPath_Edit filesep 'logs' filesep 'CalculateJobStatusMain.loginfo" 2>&1'];
                        CalculateJobStatusMainShLocation = [DestinationPath_Edit filesep 'logs' filesep 'CalculateJobStatusMain.sh'];
                        fid = fopen(CalculateJobStatusMainShLocation, 'w');
                        fprintf(fid, '%s', command);
                        ...
                        if ~exist(StatusTableFilePath, 'file')
                            command = ['"' matlabroot filesep 'bin' filesep 'matlab" -nosplash -nodesktop -r "load(''' ParameterSaveFilePath filesep ParameterSaveFileName ...
                                ''',''-mat''); addpath(genpath(PANDAPath)); CalculateJobStatusMain(DestinationPath_Edit,SubjectIDArray,dti_opt,tracking_opt,0);exit"'...
                                    ' >"' DestinationPath_Edit filesep 'logs' filesep 'CalculateJobStatusMain.loginfo" 2>&1'];
                            CalculateJobStatusMainShLocation = [DestinationPath_Edit filesep 'logs' filesep 'CalculateJobStatusMain.sh'];
                            fid = fopen(CalculateJobStatusMainShLocation, 'w');
                            fprintf(fid, '%s', command);
...
switch button
    case 'Yes'
        ...
        % Stop Monitor
        if ~isempty(JobStatusMonitorTimer)
            stop(JobStatusMonitorTimer);
        end
        % Delete Status Table
        StatusTableFilePath = [DestinationPath_Edit filesep 'logs' filesep 'StatusTable.mat'];
        if exist(StatusTableFilePath, 'file') 
