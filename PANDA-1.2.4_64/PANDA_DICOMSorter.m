function varargout = PANDA_DICOMSorter(varargin)
% GUI for DICOM Sorter (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2012
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_DICOMSORTER MATLAB code for PANDA_DICOMSorter.fig
%      PANDA_DICOMSORTER, by itself, creates a new PANDA_DICOMSORTER or raises the existing
%      singleton*.
%
%      H = PANDA_DICOMSORTER returns the handle to a new PANDA_DICOMSORTER or the handle to
%      the existing singleton*.
%
%      PANDA_DICOMSORTER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_DICOMSORTER.M with the given input arguments.
%
%      PANDA_DICOMSORTER('Property','Value',...) creates a new PANDA_DICOMSORTER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_DICOMSorter_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_DICOMSorter_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_DICOMSorter

% Last Modified by GUIDE v2.5 11-Oct-2012 14:39:07

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_DICOMSorter_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_DICOMSorter_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_DICOMSorter is made visible.
function PANDA_DICOMSorter_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_DICOMSorter (see VARARGIN)

% Choose default command line output for PANDA_DICOMSorter
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_DICOMSorter wait for user response (see UIRESUME)
% uiwait(handles.PANDADICOMSorterFigure);
set(handles.DICOMFileSuffixEdit, 'String', 'IMA');
% Set icon
SetIcon(handles);
%
TipStr = sprintf('Input all the folders containing DICOM files.');
set(handles.DICOMPathButton, 'TooltipString', TipStr);
%
TipStr = sprintf('The suffix of DICOM files.');
set(handles.DICOMFileSuffixEdit, 'TooltipString', TipStr);
%
...
function varargout = PANDA_DICOMSorter_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in DICOMPathButton.
function DICOMPathButton_Callback(hObject, eventdata, handles)
% hObject    handle to DICOMPathButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global DICOMPathList;
DICOMPathList_Button = get(hObject, 'UserData');
[a, DICOMPathList, Done] = PANDA_Select('dir', DICOMPathList_Button);
if Done == 1
    set( hObject, 'UserData', DICOMPathList);
    if ~isempty(DICOMPathList)
        DICOMPathTable = DICOMPathList;
        set( handles.DICOMPathTable, 'data', DICOMPathTable);
    else
        DICOMPathTable = cell(4,1);
        set( handles.DICOMPathTable, 'data', DICOMPathTable); 
    end
    ResizeDICOMPathTable(handles);
end



function DICOMFileSuffixEdit_Callback(hObject, eventdata, handles)
% hObject    handle to DICOMFileSuffixEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of DICOMFileSuffixEdit as text
%        str2double(get(hObject,'String')) returns contents of DICOMFileSuffixEdit as a double


% --- Executes during object creation, after setting all properties.
function DICOMFileSuffixEdit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to DICOMFileSuffixEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
global DICOMPathList;
if ~isempty(DICOMPathList)
    DICOMFileSuffix = get(handles.DICOMFileSuffixEdit, 'String');
    if ~isempty(DICOMFileSuffix)
        for i = 1:length(DICOMPathList)
            disp(['Subject ' num2str(i) ': ']);
            g_DCMsorter(DICOMPathList{i}, ['*.' DICOMFileSuffix]);
        end
        disp('Done !');
    else
        for i = 1:length(DICOMPathList)
            disp(['Subject ' num2str(i) ': ']);
            g_DCMsorter(DICOMPathList{i});
        end
        disp('Done !');
    end
else
    msgbox('Please input DICOM folders !');
end


% --- Executes when PANDADICOMSorterFigure is resized.
function PANDADICOMSorterFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDADICOMSorterFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    ResizeDICOMPathTable(handles);
...
function ResizeDICOMPathTable(handles)
DICOMFolderPath = get(handles.DICOMPathTable, 'data');
PositionFigure = get(handles.PANDADICOMSorterFigure, 'Position');
WidthCell{1} = PositionFigure(3);
WidthCell{2} = 0;
if ~isempty(DICOMFolderPath)
    for i = 1:length(DICOMFolderPath)
        tmp{i} = length(DICOMFolderPath{i}) * 8;
        tmp{i} = tmp{i} * PositionFigure(4) / 370;
    end
...
set(handles.DICOMPathTable, 'ColumnWidth', WidthCell);
...
% --- Executes when user attempts to close PANDADICOMSorterFigure.
function PANDADICOMSorterFigure_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to PANDADICOMSorterFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function DICOMPathTable_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to DICOMPathTable (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% DICOMPath = get(hObject, 'data');
% OrigineWidthCell = get(hObject, 'ColumnWidth');
% if ~isempty(DICOMPath)
%     NewWidthCell = OrigineWidthCell;
%     for i = 1:length(DICOMPath)
%         if NewWidthCell{1} < length(DICOMPath{i}) * 7.7
%             NewWidthCell{1} = length(DICOMPath{i}) * 7.7;
%         end
%     end
%     set( handles.DICOMPathTable, 'ColumnWidth', NewWidthCell);
% end
