            ...
            if strcmp(button, 'Yes')
                ...
                if exist( ParameterSaveFilePath, 'file' )
                    ...
                    for i = 1:length(DICOMNIfTI_Data_Raw_Path_Cell)
                        ...
                        tmp = [DICOMNIfTI_Data_Raw_Path_Cell{i} '  ' SubjectFolder '/'];
...
if strcmp(get(handles.RawDataPathButton, 'Enable'), 'off')
    ...
    [ParameterSaveFileName,ParameterSaveFilePath] = uigetfile({'*.PANDA_DICOMNIfTI','T1_BrainExtraction-files (*.PANDA_DICOMNIfTI)'},'Load Configuration');
...
if ~isempty(RawPathDestPath)
    ...
    for i = 1:columns
        for j = 1:rows
            ...
            tmp{j} = tmp{j} * PositionFigure(4) / 701;
        end
...
switch get(hObject, 'tag')
    ...
    case 'qsubRadio'
        ...
        DICOMNIfTI_Pipeline_opt.qsub_options = '-q all.q';
        set( handles.QsubOptionsEdit, 'Enable', 'on');
