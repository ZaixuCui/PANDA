function varargout = PANDA_FileCopier(varargin)
% GUI for copying files (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2012
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_FILECOPIER MATLAB code for PANDA_FileCopier.fig
%      PANDA_FILECOPIER, by itself, creates a new PANDA_FILECOPIER or raises the existing
%      singleton*.
%
%      H = PANDA_FILECOPIER returns the handle to a new PANDA_FILECOPIER or the handle to
%      the existing singleton*.
%
%      PANDA_FILECOPIER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_FILECOPIER.M with the given input arguments.
%
%      PANDA_FILECOPIER('Property','Value',...) creates a new PANDA_FILECOPIER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_FileCopier_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_FileCopier_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_FileCopier

% Last Modified by GUIDE v2.5 11-Oct-2012 14:39:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_FileCopier_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_FileCopier_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_FileCopier is made visible.
function PANDA_FileCopier_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_FileCopier (see VARARGIN)

% Choose default command line output for PANDA_FileCopier
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_FileCopier wait for user response (see UIRESUME)
% uiwait(handles.PANDAFilesCopyerFigure);
% Set icon
SetIcon(handles);
%
TipStr = sprintf('Input all the files to copy.');
set(handles.SourceFilesPathButton, 'TooltipString', TipStr);
%
TipStr = sprintf('The destination path for copying files.');
...
function varargout = PANDA_FileCopier_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in SourceFilesPathButton.
function SourceFilesPathButton_Callback(hObject, eventdata, handles)
% hObject    handle to SourceFilesPathButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global SourceFilesList;
SourceFilesList_Button = get(hObject, 'UserData');
[x, SourceFilesList, Done] = PANDA_Select('file', SourceFilesList_Button);
if Done == 1
    set( hObject, 'UserData', SourceFilesList );
    if ~isempty(SourceFilesList)
        SourceFilesTable = SourceFilesList;
        set( handles.SourceFilesTable, 'data', SourceFilesTable);
    else
        SourceFilesTable = cell(4,1);
        set( handles.SourceFilesTable, 'data', SourceFilesTable);
    end
    ResizeSourceFilesTable(handles);
end

% --- Executes on button press in DestinationText.
function DestinationText_Callback(hObject, eventdata, handles)
% hObject    handle to DestinationText (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
DestinationPath = get(hObject, 'String');
...
if DestinationPath
    ...
    set( handles.DestinationPathEdit, 'String', DestinationPath);
end
...
if ~isempty(SourceFilesList)
    if ~isempty(DestinationPath)
        ...
        for i = 1:length(SourceFilesList)
            try
                copyfile(SourceFilesList{i}, DestinationPath);
            catch
                disp(['Copy of ' SourceFilesList{i} ' is failed !']);
            end
            [x, FileNamePrefix, FileNameSuffix] = fileparts(SourceFilesList{i});
            DestinationFileName = [DestinationPath filesep FileNamePrefix FileNameSuffix];
            if exist(DestinationFileName, 'file')
                disp([SourceFilesList{i} ' to ' DestinationFileName]);
            end
        end
        disp('Done !');
    else
        msgbox('Please input the destination path of the files !');
    end
else
    msgbox('Please input source files to be removed !');
end
...
% --- Executes when PANDAFilesCopyerFigure is resized.
function PANDAFilesCopyerFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDAFilesCopyerFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    ResizeSourceFilesTable(handles);
...
function ResizeSourceFilesTable(handles)
SourceFilePath = get(handles.SourceFilesTable, 'data');
PositionFigure = get(handles.PANDAFilesCopyerFigure, 'Position');
WidthCell{1} = PositionFigure(3);
WidthCell{2} = 0;
if ~isempty(SourceFilePath)
    for i = 1:length(SourceFilePath)
        tmp{i} = length(SourceFilePath{i}) * 8;
...
set(handles.SourceFilesTable, 'ColumnWidth', WidthCell);


% --- Executes when user attempts to close PANDAFilesCopyerFigure.
function PANDAFilesCopyerFigure_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to PANDAFilesCopyerFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
clear global SourceFilesList;
clear global DestinationPath;
delete(hObject);


% --------------------------------------------------------------------
function SourceFilesTable_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to SourceFilesTable (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% FilesPath = get(hObject, 'data');
% OrigineWidthCell = get(hObject, 'ColumnWidth');
% if ~isempty(FilesPath)
%     NewWidthCell = OrigineWidthCell;
%     for i = 1:length(FilesPath)
%         if NewWidthCell{1} < length(FilesPath{i}) * 7.7
%             NewWidthCell{1} = length(FilesPath{i}) * 7.7;
%         end
%     end
%     set( handles.SourceFilesTable, 'ColumnWidth', NewWidthCell);
% end
