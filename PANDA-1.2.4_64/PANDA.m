        ...
        if ~LogPathPermissionDenied
            ...
            switch button
                case 'Yes'
                    ...
                    fclose(fid);
%                     instr_batch = ['at -f "' DTIPipelineShLocation '" now'];
%                     system(instr_batch);
...
if exist( StatusFilePath, 'file' )
    try
        ...
        JobName = [JobName, {'EDDYCURRENT', 'average', 'BET_2', 'merge', 'dtifit', 'LDH'}];
        if dti_opt.Normalizing_Flag
            JobName = [JobName, {'BeforeNormalize_FA', 'BeforeNormalize_MD', 'BeforeNormalize_L1','BeforeNormalize_L23m',...
                'BeforeNormalize_LDHs', 'BeforeNormalize_LDHk', 'FAnormalize', 'applywarp_FA_1mm', 'applywarp_MD_1mm', ...
                'applywarp_L1_1mm', 'applywarp_L23m_1mm', 'applywarp_LDHs_1mm', 'applywarp_LDHk_1mm'}];
        end
        ...
        if dti_opt.Smoothing_Flag
            JobName = [JobName, {['smoothNII_FA_' num2str(dti_opt.applywarp_2_ref_fileName) 'mm'], ...
                ...
                ['smoothNII_LDHs_' num2str(dti_opt.applywarp_8_ref_fileName) 'mm'], ...
                ['smoothNII_LDHk_' num2str(dti_opt.applywarp_8_ref_fileName) 'mm']}];
        end
        if dti_opt.Atlas_Flag
            JobName = [JobName, {'atlas_FA_1mm', 'atlas_MD_1mm', 'atlas_L1_1mm', 'atlas_L23m_1mm', 'atlas_LDHs_1mm', 'atlas_LDHk_1mm'}];
        end
        JobName = [JobName, {'delete_tmp_file'}];

        if dti_opt.TBSS_Flag == 1
            JobName = [JobName, {'TBSSDismap', 'skeleton_FA', 'skeleton_MD', 'skeleton_L1', 'skeleton_L23m', 'skeleton_LDHs', 'skeleton_LDHk'}];
        end
        ...
        if dti_opt.Atlas_Flag
            JobName = [JobName, {'Split_WMLabel', 'Split_WMProbtract', 'ExportAtlasResults_FA_ToExcel', ...
                'ExportAtlasResults_MD_ToExcel', 'ExportAtlasResults_L1_ToExcel', 'ExportAtlasResults_L23m_ToExcel'}];
        end
        if dti_opt.TBSS_Flag == 1
