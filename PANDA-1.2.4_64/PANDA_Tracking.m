...
% Last Modified by GUIDE v2.5 11-Oct-2012 13:04:43
...
if ~isempty(SubjectFoldersID)
    ...
    for i = 1:columns
        for j = 1:rows
            tmp{j} = length(SubjectFoldersID{j, i}) * 8;
            tmp{j} = tmp{j} * PositionFigure(4) / 768;
        end
...
if isempty(NativePathCellTracking)
    ...
    if ParameterSaveFileName
        cmdString = ['tmp = load(''' ParameterSaveFilePath filesep ParameterSaveFileName ''', ''-mat'');'];
        eval( cmdString );
        SubjectIDArrayTmp = tmp.SubjectIDArray;
