...
if length(PreviousDirCell) > 20
    tmp_PANDA = PreviousDirCell;
    PreviousDirCell = '';
    j = 0;
    for i = (length(tmp_PANDA) - 19):length(tmp_PANDA)
        j = j + 1;
        PreviousDirCell{j} = tmp_PANDA{i};
    end
end
...
global PreviousDir;
global Type;
tmp_PANDA = get(handles.CurrentDirEdit, 'String');
if isempty(strfind(tmp_PANDA, '*'))
    PreviousDir = tmp_PANDA;
end
