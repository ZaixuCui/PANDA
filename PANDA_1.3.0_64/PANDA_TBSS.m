...
% Last Modified by GUIDE v2.5 05-Jul-2015 07:03:19
...
global TBSSStopFlag;
global PANDAPath;
global TBSSPANDAConfigFlag;
global TBSSResultPath;
global TBSS_WM_Label_Atlas;
global TBSS_WM_Probtract_Atlas;
global PANDA_Configuration_Path;
...
FADataPathCellTBSS = '';
ThresholdTBSS = 0.2;
TBSSPANDAConfigFlag = 1;
TBSSResultPath = '';
PANDA_Configuration_Path = '';
set(handles.PANDAConfig_CheckBox, 'Value', 1);
set(handles.PANDAConfig_Edit, 'Enable', 'on');
set(handles.PANDAConfig_Edit, 'String', PANDA_Configuration_Path);
set(handles.PANDAConfig_Button, 'Enable', 'on');
...
% WM atlas
TBSS_WM_Label_Atlas = [PANDAPath filesep 'data' filesep 'atlases' filesep 'rICBM_DTI' filesep 'rICBM_DTI_81_WMPM_FMRIB58.nii.gz'];
set( handles.WMLabelAtlas_Edit, 'Enable', 'on' );
set( handles.WMLabelAtlas_Edit, 'string', TBSS_WM_Label_Atlas );
set( handles.WMLabelAtlas_Button, 'Enable', 'on' );
TBSS_WM_Probtract_Atlas = [PANDAPath filesep 'data' filesep 'atlases' filesep 'rICBM_DTI' filesep 'JHU_ICBM_tracts_maxprob_thr25_1mm.nii.gz'];
set( handles.WMProbtractAtlas_Edit, 'Enable', 'on' );
set( handles.WMProbtractAtlas_Edit, 'string', TBSS_WM_Probtract_Atlas );
set( handles.WMProbtractAtlas_Button, 'Enable', 'on' );
% Set icon
SetIcon(handles);
%
...
elseif isempty(TBSS_WM_Label_Atlas)
    msgbox('Please input white matter altas1!');
elseif isempty(TBSS_WM_Probtract_Atlas)
    msgbox('Please input white matter altas2!');
elseif exist(LockFilePath, 'file') | strcmp(get(handles.PANDAConfig_CheckBox, 'Enable'), 'off')
    ...
    if ~LogPathPermissionDenied
        ...
        if strcmp(button, 'Yes')
            ...
            cmdString = [ 'save ' ParameterSaveFilePath ' FAPathCellTBSS' ' DataPathCellTBSS' ' TBSSPipeline_opt' ...
                ' ThresholdTBSS' ' TBSSResultPath' ' TBSSPANDAConfigFlag' ' PANDA_Configuration_Path' ' TBSS_WM_Label_Atlas' ' TBSS_WM_Probtract_Atlas' ' PANDAPath'];
            eval(cmdString);
            ...
            if exist( ParameterSaveFilePath, 'file' )
                ...
                command = ['"' matlabroot filesep 'bin' filesep 'matlab" -nosplash -nodesktop -r "load(''' ParameterSaveFilePath ''',''-mat'');'...
                    'addpath(genpath(PANDAPath)); pipeline=g_TBSS_pipeline( FAPathCellTBSS, DataPathCellTBSS, ThresholdTBSS, TBSS_WM_Label_Atlas, TBSS_WM_Probtract_Atlas, TBSSResultPath, TBSSPipeline_opt );exit"'...
                ...
                system([ShPath(1:end-1) ' ' TBSSPipelineShLocation ' &']);
                % Set edit box and button unable
                set(handles.PANDAConfig_CheckBox, 'Enable', 'off');
                set(handles.PANDAConfig_Edit, 'Enable', 'off');
                set(handles.PANDAConfig_Button, 'Enable', 'off');
                set(handles.FAPathButton, 'Enable', 'off');
                set(handles.DataPathButton, 'Enable', 'off');
                set(handles.ThresholdEdit, 'Enable', 'off');
                set(handles.TBSSResultPathEdit, 'Enable', 'off');
                set(handles.TBSSResultPathButton, 'Enable', 'off');
                set(handles.WMLabelAtlas_Edit, 'Enable', 'off');
                set(handles.WMLabelAtlas_Button, 'Enable', 'off');
                set(handles.WMProbtractAtlas_Edit, 'Enable', 'off');
                set(handles.WMProbtractAtlas_Button, 'Enable', 'off');
                ...
                if ~exist(StatusFilePath)
                    ...
                    for i = 1:length(FAPathCellTBSS)
                        ...
                        JobsStatus{i, 4} = num2str(cols * 4 + 3); % For each parameter type, four jobs 
                    end
                ...
                TBSSStopFlag = '';
                JobStatusMonitorTimerTBSS = timer( 'TimerFcn', {@JobStatusMonitorTBSS, handles}, 'period', 7, 'ExecutionMode', 'fixedRate');
...
switch get(hObject, 'tag')
    ...
    case 'qsubRadio'
        ...
        TBSSPipeline_opt.qsub_options = '-V -q all.q';
...
switch button
    case 'Yes'
        ...
        set(handles.PANDAConfig_CheckBox, 'Enable', 'on');
        TBSSPANDAConfigFlag = 1;
        set(handles.PANDAConfig_CheckBox, 'Value', 1);
        set(handles.PANDAConfig_Edit, 'Enable', 'on');
        set(handles.PANDAConfig_Edit, 'String', '')
        set(handles.PANDAConfig_Button, 'Enable', 'on');
        set(handles.FAPathButton, 'Enable', 'off');
        set(handles.DataPathButton, 'Enable', 'off');
...
if strcmp(get(handles.PANDAConfig_CheckBox, 'Enable'), 'off')
    ...
    if ParameterSaveFileName ~= 0
        ...
        if ~isempty(TBSSResultPath) & exist(LockFilePath, 'file')
            ...
            set( handles.WMProbtractAtlas_Button, 'Enable', 'on' );
            set(handles.PANDAConfig_Button, 'Enable', 'on');
            if TBSSPANDAConfigFlag
                set(handles.PANDAConfig_CheckBox, 'Value', 1);
                set(handles.PANDAConfig_Edit, 'String', PANDA_Configuration_Path);
            else
                set(handles.PANDAConfig_CheckBox, 'Value', 0);
...
if exist( StatusFilePath, 'file' )
    ...
    try    
        ...
        JobName = {'g_dismap', 'Split_WMLabel', 'Split_WMProbtract'};
        [rows, cols] = size(DataPathCellTBSS);
        for i = 1:cols  % cols is the quantity of data type to be projected to the skeleton
            JobName = [JobName, [ 'skeleton_Data', num2str(i) ], [ 'skeleton_atlas_Data', num2str(i) ], ...
                [ 'MergeSkeleton_Data', num2str(i) ], [ 'ExportAtlasResults_Skeleton_ToExcel', num2str(i) ]];
        end
        ...
        % Calculate job status of the ith subject 
            ...
            for j = 1:JobQuantity
                ...
                if strcmp(JobName{j}, 'g_dismap') | strcmp(JobName{j}(1:end - 1), 'MergeSkeleton_Data') ...
                        | strcmp(JobName{j}(1:end - 1),'ExportAtlasResults_Skeleton_ToExcel') ...
...
if ~isempty(FAPathDataPathCell)
    ...
    for i = 1:columns
        ...
        for j = 1:rows
            tmp_PANDA{j} = length(FAPathDataPathCell{j, i}) * 8;
            tmp_PANDA{j} = tmp_PANDA{j} * PositionFigure(4) / 610;
        end
...
% --- Executes on button press in PANDAConfig_CheckBox.
function PANDAConfig_CheckBox_Callback(hObject, eventdata, handles)
% hObject    handle to PANDAConfig_CheckBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of PANDAConfig_CheckBox
global TBSSPANDAConfigFlag;
global FAPathCellTBSS;
global FADataPathCellTBSS;
global DataPathCellTBSS;
...
function PANDAConfig_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to PANDAConfig_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of PANDAConfig_Edit as text
%        str2double(get(hObject,'String')) returns contents of PANDAConfig_Edit as a double


% --- Executes during object creation, after setting all properties.
function PANDAConfig_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to PANDAConfig_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in PANDAConfig_Button.
function PANDAConfig_Button_Callback(hObject, eventdata, handles)
% hObject    handle to PANDAConfig_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global FAPathCellTBSS;
global FADataPathCellTBSS;
...
PANDA_Configuration_Path = [ParameterSaveFilePath ParameterSaveFileName];
if ParameterSaveFileName
    cmdString = ['tmp_PANDA = load(''' PANDA_Configuration_Path ''', ''-mat'');'];
    eval(cmdString);
    set(handles.PANDAConfig_Edit, 'String', PANDA_Configuration_Path);
    SubjectIDArrayTmp = tmp_PANDA.SubjectIDArray;
    ResultPathTmp = tmp_PANDA.DestinationPath_Edit;
    PrefixTmp = tmp_PANDA.TensorPrefixEdit;
    if ~isempty(PrefixTmp)
        PrefixTmp = [PrefixTmp '_'];
    end
    for i = 1:length(SubjectIDArrayTmp)
        FAPathCellTBSS{i} = [ResultPathTmp filesep num2str(SubjectIDArrayTmp(i), '%05d') filesep 'standard_space' ...
            filesep PrefixTmp num2str(SubjectIDArrayTmp(i), '%05d') '_FA_4normalize_to_target_1mm.nii.gz'];
        MDPathCellTBSS{i} = [ResultPathTmp filesep num2str(SubjectIDArrayTmp(i), '%05d') filesep 'standard_space' ...
            filesep PrefixTmp num2str(SubjectIDArrayTmp(i), '%05d') '_MD_4normalize_to_target_1mm.nii.gz'];
        L1PathCellTBSS{i} = [ResultPathTmp filesep num2str(SubjectIDArrayTmp(i), '%05d') filesep 'standard_space' ...
            filesep PrefixTmp num2str(SubjectIDArrayTmp(i), '%05d') '_L1_4normalize_to_target_1mm.nii.gz'];
        L23mPathCellTBSS{i} = [ResultPathTmp filesep num2str(SubjectIDArrayTmp(i), '%05d') filesep 'standard_space' ...
            filesep PrefixTmp num2str(SubjectIDArrayTmp(i), '%05d') '_L23m_4normalize_to_target_1mm.nii.gz'];
        if tmp_PANDA.dti_opt.LDH_Flag
            LDHsPathCellTBSS{i} = [ResultPathTmp filesep num2str(SubjectIDArrayTmp(i), '%05d') filesep 'standard_space' ...
                filesep PrefixTmp num2str(SubjectIDArrayTmp(i), '%05d') '_' ...
                num2str(tmp_PANDA.dti_opt.LDH_Neighborhood-1, '%02d') 'LDHs_4normalize_to_target_1mm.nii.gz'];
            LDHkPathCellTBSS{i} = [ResultPathTmp filesep num2str(SubjectIDArrayTmp(i), '%05d') filesep 'standard_space' ...
                filesep PrefixTmp num2str(SubjectIDArrayTmp(i), '%05d') '_' ... 
                num2str(tmp_PANDA.dti_opt.LDH_Neighborhood, '%02d') 'LDHk_4normalize_to_target_1mm.nii.gz'];
        end
    end
    FAPathCellTBSS = FAPathCellTBSS';
    DataPathCellTBSS = [FAPathCellTBSS MDPathCellTBSS' L1PathCellTBSS' L23mPathCellTBSS'];
    if tmp_PANDA.dti_opt.LDH_Flag
        DataPathCellTBSS = [DataPathCellTBSS LDHsPathCellTBSS' LDHkPathCellTBSS'];
    end
    FADataPathCellTBSS = [FAPathCellTBSS DataPathCellTBSS];
    [rows, cols] = size(FADataPathCellTBSS);
    ColumnNameCell{1} = 'FA Path (1*1*1)';
    for j = 2:cols
        ColumnNameCell{j} = ['Data' num2str(j - 1) ' Path (1*1*1)'];
    end
    set( handles.FADataPathTable, 'data', FADataPathCellTBSS, 'ColumnName', ColumnNameCell );
    ResizeFADataTable(handles);
end
...
% --- Executes on button press in WMLabelAtlas_Button.
function WMLabelAtlas_Button_Callback(hObject, eventdata, handles)
% hObject    handle to WMLabelAtlas_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global TBSS_WM_Label_Atlas;
[WMAtlas_FolderName ParentFolder] = uigetfile({'*.nii.gz','NIfTI-files (*.nii.gz)'});
WMAtlas_FolderPath = [ParentFolder WMAtlas_FolderName];
if ~isnumeric(WMAtlas_FolderPath)
    set( handles.WMLabelAtlas_Edit, 'string', WMAtlas_FolderPath );
    TBSS_WM_Label_Atlas = WMAtlas_FolderPath;
end


% --- Executes on button press in WMProbtractAtlas_Button.
function WMProbtractAtlas_Button_Callback(hObject, eventdata, handles)
% hObject    handle to WMProbtractAtlas_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global TBSS_WM_Probtract_Atlas;
[WMAtlas_FolderName ParentFolder] = uigetfile({'*.nii.gz','NIfTI-files (*.nii.gz)'});
WMAtlas_FolderPath = [ParentFolder WMAtlas_FolderName];
if ~isnumeric(WMAtlas_FolderPath)
    set( handles.WMProbtractAtlas_Edit, 'string', WMAtlas_FolderPath );
    TBSS_WM_Probtract_Atlas = WMAtlas_FolderPath;
end



function WMLabelAtlas_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to WMLabelAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of WMLabelAtlas_Edit as text
%        str2double(get(hObject,'String')) returns contents of WMLabelAtlas_Edit as a double
global TBSS_WM_Label_Atlas;
set(hObject, 'String', TBSS_WM_Label_Atlas);


% --- Executes during object creation, after setting all properties.
function WMLabelAtlas_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to WMLabelAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function WMProbtractAtlas_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to WMProbtractAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of WMProbtractAtlas_Edit as text
%        str2double(get(hObject,'String')) returns contents of WMProbtractAtlas_Edit as a double
global TBSS_WM_Probtract_Atlas;
set(hObject, 'String', TBSS_WM_Probtract_Atlas);


% --- Executes during object creation, after setting all properties.
function WMProbtractAtlas_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to WMProbtractAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
