...
% Last Modified by GUIDE v2.5 15-Jul-2015 22:15:57
...
elseif isempty(TBSSRandomise_ResultPath)
    ...
    [~, dim4_str] = system(['fslval ' TBSSRandomise_Skeleton ' dim4']);
    dim4 = str2num(dim4_str);
...
%         disp(OutputInformation);
        TBSSRandomise_OutputInfo_All = [TBSSRandomise_OutputInfo_All; {OutputInformation}];
        if length(TBSSRandomise_OutputInfo_All) > 21
            StatusInfo = '... ...';
            StatusInfo = [StatusInfo; '... ...'];
            StatusInfo = [StatusInfo; '... ...'];
            StatusInfo = [StatusInfo; TBSSRandomise_OutputInfo_All([end - 19:end])];
        else
            StatusInfo = TBSSRandomise_OutputInfo_All;
        end
        set(handles.Randomise_StatusList, 'Value', 1);
        set(handles.Randomise_StatusList, 'String', StatusInfo);
        set(handles.Randomise_StatusList, 'ListboxTop', length(StatusInfo));
...
% --- Executes on selection change in Randomise_StatusList.
function Randomise_StatusList_Callback(hObject, eventdata, handles)
% hObject    handle to Randomise_StatusList (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns Randomise_StatusList contents as cell array
%        contents{get(hObject,'Value')} returns selected item from Randomise_StatusList


% --- Executes during object creation, after setting all properties.
function Randomise_StatusList_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Randomise_StatusList (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
switch button
    case 'Yes'
        ...
        TBSSRandomise_OutputInfo_All = [];
        set(handles.Randomise_StatusList, 'Value', 1);
        set(handles.Randomise_StatusList, 'String', '');
        set(handles.TBSS_Statistic_Run, 'Enable', 'on');
        set(handles.LoadButton, 'Enable', 'on');
        set(handles.TerminateButton, 'Enable', 'on');
    case 'No'
        return;
end
