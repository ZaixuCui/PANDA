...
% Last Modified by GUIDE v2.5 11-Jun-2015 10:54:34
...
if ~isfield(dti_opt, 'LDH_Flag')
    dti_opt.LDH_Flag = 1;
    set( handles.LDHCheckBox, 'Value', 1);
...
elseif ~dti_opt.LDH_Flag
    set( handles.SevenRadio, 'Enable', 'of' );
...
if exist( LockFilePath, 'file' )
    ...
    set( handles.NIIcropSliceGapEdit, 'Enable', 'off' );
    %
    set( handles.LDHCheckBox, 'Enable', 'off');
...
if LockFlag == 0
    ...
    if DiffusionOpt_ParametersChangeFlag
        ...
        Info_Quantity = Info_Quantity + 4;
        if dti_opt.LDH_Flag
            Info_Quantity = Info_Quantity + 1;
            Check_info{Info_Quantity} = [' LDH Neighborhood = ' num2str(dti_opt.LDH_Neighborhood)];
        end
    %     Info_Quantity = Info_Quantity + 1;
...
% --- Executes on button press in LDHCheckBox.
function LDHCheckBox_Callback(hObject, eventdata, handles)
% hObject    handle to LDHCheckBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of LDHCheckBox
global dti_opt;
global DiffusionOpt_ParametersChangeFlag;
LDH_Flag_Previous = dti_opt.LDH_Flag;
if get(hObject, 'value')
    dti_opt.LDH_Flag = 1;
    set( handles.LDHCheckBox, 'Value', 1);
    set( handles.SevenRadio, 'Enable', 'on' );
...
if LDH_Flag_Previous ~= dti_opt.LDH_Flag
    DiffusionOpt_ParametersChangeFlag = 1;
end
