function varargout = PANDA_FDRCorr(varargin)
% PANDA_FDRCORR MATLAB code for PANDA_FDRCorr.fig
%      PANDA_FDRCORR, by itself, creates a new PANDA_FDRCORR or raises the existing
%      singleton*.
%
%      H = PANDA_FDRCORR returns the handle to a new PANDA_FDRCORR or the handle to
%      the existing singleton*.
%
%      PANDA_FDRCORR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_FDRCORR.M with the given input arguments.
%
%      PANDA_FDRCORR('Property','Value',...) creates a new PANDA_FDRCORR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_FDRCorr_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_FDRCorr_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_FDRCorr

% Last Modified by GUIDE v2.5 17-Jul-2015 22:58:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_FDRCorr_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_FDRCorr_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_FDRCorr is made visible.
function PANDA_FDRCorr_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_FDRCorr (see VARARGIN)

% Choose default command line output for PANDA_FDRCorr
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_FDRCorr wait for user response (see UIRESUME)
% uiwait(handles.figure1);
global PANDAFDR_PValue;
global PANDAFDR_QValue;
global PANDAFDR_ResultPath;
PANDAFDR_PValue = '';
set(handles.PValue_Edit, 'String', '');
PANDAFDR_QValue = 0.05;
set(handles.QValue_Edit, 'String', '0.05');
PANDAFDR_ResultPath = '';
...
function varargout = PANDA_FDRCorr_OutputFcn(hObject, eventdata, handles) 
...
global PANDAFDR_QValue;
global PANDAFDR_ResultPath;
if isempty(PANDAFDR_PValue)
    msgbox('Please input P value file (.txt)!');
elseif ~exist(PANDAFDR_PValue, 'file')
    msgbox('The P value file is not exist!');
elseif isempty(PANDAFDR_QValue);
    msgbox('Please input Q value!');
elseif isempty(PANDAFDR_ResultPath)
    msgbox('Please input the resultant folder path!');
else
    try
        PValues = load(PANDAFDR_PValue, '-ascii');
    catch
        msgbox('The P value file is illegal, it should be only one column!');
        return;
    end
    if ~exist(PANDAFDR_ResultPath, 'dir')
        mkdir(PANDAFDR_ResultPath);
    end
    ResultantFile = [PANDAFDR_ResultPath filesep 'FDR_Correction.txt'];
    [Pthreshold RetainID] = g_FDRCorr(PValues', PANDAFDR_QValue)
    fid = fopen(ResultantFile, 'w');
    fprintf(fid, 'FDR correction, 0.05:\n');
    fprintf(fid, '%s\n', num2str(Pthreshold));
    fprintf(fid, 'Significant index:\n');
    fprintf(fid, '%s\n', mat2str(RetainID));
    fclose(fid);
end
...
% Hints: get(hObject,'String') returns contents of PValue_Edit as text
%        str2double(get(hObject,'String')) returns contents of PValue_Edit as a double
global PANDAFDR_PValue;
set(handles.PValue_Edit, 'String', PANDAFDR_PValue);
...
% --- Executes on button press in PValue_Button.
function PValue_Button_Callback(hObject, eventdata, handles)
% hObject    handle to PValue_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global PANDAFDR_PValue;
[PValue_FileName, PValue_ParentFolder] = uigetfile({'*.txt','P Value (*.txt)'},'Input P value file');
if ~isnumeric(PValue_FileName)
    PANDAFDR_PValue = [PValue_ParentFolder PValue_FileName];
    set(handles.PValue_Edit, 'String', PANDAFDR_PValue);
end



function QValue_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to QValue_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of QValue_Edit as text
%        str2double(get(hObject,'String')) returns contents of QValue_Edit as a double
global PANDAFDR_QValue;
QValue_Edit = get(hObject, 'string');
PANDAFDR_QValue = str2num(QValue_Edit);
if isempty(PANDAFDR_QValue)
    QValue_Edit = '';
    set(hObject, 'string', '');
...
% --- Executes during object creation, after setting all properties.
function QValue_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to QValue_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
if ~isnumeric(ResultPath)
    PANDAFDR_ResultPath = ResultPath;
    set(handles.ResultPath_Edit, 'String', PANDAFDR_ResultPath);
end
...
global PANDAFDR_ResultPath;
PANDAFDR_ResultPath = get( hObject, 'string' );
