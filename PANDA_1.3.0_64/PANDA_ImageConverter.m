function varargout = PANDA_ImageConverter(varargin)
% GUI for Converting Image Format (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_IMAGECONVERTER MATLAB code for PANDA_ImageConverter.fig
%      PANDA_IMAGECONVERTER, by itself, creates a new PANDA_IMAGECONVERTER or raises the existing
%      singleton*.
%
%      H = PANDA_IMAGECONVERTER returns the handle to a new PANDA_IMAGECONVERTER or the handle to
%      the existing singleton*.
%
%      PANDA_IMAGECONVERTER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_IMAGECONVERTER.M with the given input arguments.
%
%      PANDA_IMAGECONVERTER('Property','Value',...) creates a new PANDA_IMAGECONVERTER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_ImageConverter_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_ImageConverter_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_ImageConverter

% Last Modified by GUIDE v2.5 11-Oct-2012 15:46:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_ImageConverter_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_ImageConverter_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_ImageConverter is made visible.
function PANDA_ImageConverter_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_ImageConverter (see VARARGIN)

% Choose default command line output for PANDA_ImageConverter
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_ImageConverter wait for user response (see UIRESUME)
% uiwait(handles.PANDAImageFormatConvertFigure);
% Set icon
SetIcon(handles);
%
TipStr = sprintf('Input all the images to convert.');
set(handles.ImageFilesPathButton, 'TooltipString', TipStr);
%
TipStr = sprintf('Select the final type of the images.');
set(handles.ResultTypePopupmenu, 'TooltipString', TipStr);
%
...
function varargout = PANDA_ImageConverter_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in ImageFilesPathButton.
function ImageFilesPathButton_Callback(hObject, eventdata, handles)
% hObject    handle to ImageFilesPathButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global ImageFilesList;
ImageFilesList_Button = get(hObject, 'UserData');
[a, ImageFilesList, Done] = PANDA_Select('imgAll', ImageFilesList_Button);
if Done == 1
    set( hObject, 'UserData', ImageFilesList );
    if ~isempty(ImageFilesList)
        ImageFilesTable = ImageFilesList;
        set( handles.ImageFilesPathTable, 'data', ImageFilesTable);
    else
        ImageFilesTable = cell(4,1);
        set( handles.ImageFilesPathTable, 'data', ImageFilesTable);  
    end
    ResizeImageFilesPathTable(handles);
end


% --- Executes on selection change in ResultTypePopupmenu.
function ResultTypePopupmenu_Callback(hObject, eventdata, handles)
% hObject    handle to ResultTypePopupmenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns ResultTypePopupmenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from ResultTypePopupmenu


% --- Executes during object creation, after setting all properties.
function ResultTypePopupmenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ResultTypePopupmenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
global ImageFilesList;
if ~isempty(ImageFilesList)
    ResultFormatStringList = get(handles.ResultTypePopupmenu, 'String');
    SelectedResultFormatID = get(handles.ResultTypePopupmenu, 'Value');
    ResultFormat = ResultFormatStringList{SelectedResultFormatID};
    for i = 1:length(ImageFilesList)
        msg = '';
        [ImageDir, ImageFileName, Suffix] = fileparts(ImageFilesList{i});
        try  
            if strcmp(Suffix, '.gz')
                cmd = ['fslchfiletype ' ResultFormat ' ' ImageFilesList{i}];
                system(cmd);
            elseif strcmp(Suffix, '.nii') | strcmp(Suffix, '.img')
                cmd = ['fslchfiletype ' ResultFormat ' ' ImageFilesList{i}];
                system(cmd);
            else
                msg = ['The format of ' ImageFilesList{i} ' is illegal ! Please input .nii, .nii.gz or .img files'];
            end
        catch
            msg = ['The convert of ' ImageFilesList{i} 'is failed ! Please check !'];
        end
        %Judge whether the file is created
        if isempty(msg)
            if strcmp(Suffix, '.gz')
                [x, ImageFileName, y] = fileparts(ImageFileName);
            end
            if strcmp(ResultFormat, 'NIFTI')
                ResultFileName = [ImageDir filesep ImageFileName '.nii'];
            elseif strcmp(ResultFormat, 'NIFTI_GZ')
                ResultFileName = [ImageDir filesep ImageFileName '.nii.gz'];
            else
                ResultFileName = [ImageDir filesep ImageFileName '.img'];
            end
            if exist(ResultFileName, 'file')
                msg = ['The convert of ' ImageFilesList{i} ' is done !'];
            else
                msg = ['The convert of ' ImageFilesList{i} ' is failed ! Please check !'];
            end
        end
    ...
    msgbox('Please input image files to be converted !');
end


% --- Executes when user attempts to close PANDAImageFormatConvertFigure.
function PANDAImageFormatConvertFigure_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to PANDAImageFormatConvertFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
clear global ImageFilesList;
delete(hObject);


% --- Executes when PANDAImageFormatConvertFigure is resized.
function PANDAImageFormatConvertFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDAImageFormatConvertFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
...
function ResizeImageFilesPathTable(handles)
ImagePathList = get(handles.ImageFilesPathTable, 'data');
PositionFigure = get(handles.PANDAImageFormatConvertFigure, 'Position');
WidthCell{1} = PositionFigure(3);
WidthCell{2} = 0;
if ~isempty(ImagePathList)
    for i = 1:length(ImagePathList)
        tmp_PANDA{i} = length(ImagePathList{i}) * 8;
        tmp_PANDA{i} = tmp_PANDA{i} * PositionFigure(4) / 351;
    end
...
set(handles.ImageFilesPathTable, 'ColumnWidth', WidthCell);


% --- Executes on button press in ResultFormatText.
function ResultFormatText_Callback(hObject, eventdata, handles)
% hObject    handle to ResultFormatText (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function ImageFilesPathTable_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to ImageFilesPathTable (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% ImagesPath = get(hObject, 'data');
% OrigineWidthCell = get(hObject, 'ColumnWidth');
% if ~isempty(ImagesPath)
%     NewWidthCell = OrigineWidthCell;
%     for i = 1:length(ImagesPath)
%         if NewWidthCell{1} < length(ImagesPath{i}) * 7.7
%             NewWidthCell{1} = length(ImagesPath{i}) * 7.7;
%         end
%     end
%     set( handles.ImageFilesPathTable, 'ColumnWidth', NewWidthCell);
% end
