function varargout = PANDA_ClusterLocater(varargin)
% GUI for reporting white matter regions, by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
%      Last edited  
%--------------------------------------------------------------------------
% PANDA_CLUSTERLOCATER MATLAB code for PANDA_ClusterLocater.fig
%      PANDA_CLUSTERLOCATER, by itself, creates a new PANDA_CLUSTERLOCATER or raises the existing
%      singleton*.
%
%      H = PANDA_CLUSTERLOCATER returns the handle to a new PANDA_CLUSTERLOCATER or the handle to
%      the existing singleton*.
%
%      PANDA_CLUSTERLOCATER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_CLUSTERLOCATER.M with the given input arguments.
%
%      PANDA_CLUSTERLOCATER('Property','Value',...) creates a new PANDA_CLUSTERLOCATER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_ClusterLocater_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_ClusterLocater_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_ClusterLocater

% Last Modified by GUIDE v2.5 22-Jul-2015 21:54:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_ClusterLocater_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_ClusterLocater_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_ClusterLocater is made visible.
function PANDA_ClusterLocater_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_ClusterLocater (see VARARGIN)

% Choose default command line output for PANDA_ClusterLocater
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_ClusterLocater wait for user response (see UIRESUME)
% uiwait(handles.PANDAClusterLocaterFigure);
global AtlasFlag_Report;
global AtlasPath_Report;
global AtlasInfo_Report;
global AtlasPathMenu;
global AtlasInfoMenu;
% Set icon
SetIcon(handles);
%
...
WidthCell{1} = 450;
WidthCell{2} = 0;
set( handles.ClusterImgTable, 'ColumnWidth', WidthCell);
% 
AtlasFlag_Report = 1;
AtlasName_Report = 'JHU ICBM-DTI-81 White-Matter Labels';
[PANDAPath, ~, ~] = fileparts(which('PANDA'));
% path of atlases
AtlasPathMenu{1} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'rICBM_DTI' filesep 'rICBM_DTI_81_WMPM_FMRIB58.nii.gz'];
AtlasPathMenu{2} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'rICBM_DTI' filesep 'JHU_ICBM_tracts_maxprob_thr25_1mm.nii.gz'];
AtlasPathMenu{3} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'ForReport' filesep 'AAL_1mm.nii.gz'];
AtlasPathMenu{4} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'ForReport' filesep 'HOA_112_1mm.nii'];
AtlasPathMenu{5} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'ForReport' filesep 'brodmann.nii.gz'];
AtlasPath_Report = AtlasPathMenu{1};
% infomation of atlases
AtlasInfoMenu{1} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'rICBM_DTI' filesep 'WM_label_BrainRegion_50.mat'];
AtlasInfoMenu{2} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'rICBM_DTI' filesep 'WM_tract_BrainRegion.mat'];
AtlasInfoMenu{3} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'ForReport' filesep 'AAL_Label.mat'];
AtlasInfoMenu{4} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'ForReport' filesep 'HOA_Label.mat'];
AtlasInfoMenu{5} = [PANDAPath filesep 'data' filesep 'atlases' filesep 'ForReport' filesep 'Brodmann_Label.mat'];
AtlasInfo_Report = AtlasInfoMenu{1};
set(handles.AtlasCheckBox, 'Value', 1);
set(handles.AtlasMenu, 'Value', 1);
set(handles.Atlas_Edit, 'Enable', 'off');
set(handles.Atlas_Button, 'Enable', 'off');


% --- Outputs from this function are returned to the command line.
function varargout = PANDA_ClusterLocater_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in ClusterImage_Button.
function ClusterImage_Button_Callback(hObject, eventdata, handles)
% hObject    handle to ClusterImage_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global ClusterImgPathList;
ClusterImgPathList_Button = get(hObject, 'UserData');
[x, ClusterImgPathList, Done] = PANDA_Select('img', ClusterImgPathList_Button);
if Done == 1
    set( hObject, 'UserData', ClusterImgPathList );
    if ~isempty(ClusterImgPathList)
        ClusterImgPathTable = ClusterImgPathList;
        set( handles.ClusterImgTable, 'data', ClusterImgPathTable);
    else
        ClusterImgPathTable = cell(4,1);
        set( handles.ClusterImgTable, 'data', ClusterImgPathTable);
    end
end
...
% handles    structure with handles and user data (see GUIDATA)
global AtlasPath_Report;
[AtlasName,AtlasParent] = uigetfile({'*.nii;*.nii.gz;*.img','NIfTI-files (*.nii,*nii.gz,*.img)'});
if AtlasParent ~= 0
    AtlasPath_Report = [AtlasParent AtlasName];
    set(handles.Atlas_Edit, 'string', AtlasPath_Report);
end
...
global ResultantFolder_Report;
ResultantFolder_Report = get(hObject, 'String');
if ~exist(ResultantFolder_Report, 'dir')
    try
        mkdir(ResultantFolder_Report);
    catch
...
% handles    structure with handles and user data (see GUIDATA)
global ResultantFolder_Report;
ResultantFolder_Report = uigetdir;
if ResultantFolder_Report
    if ~exist(ResultantFolder_Report, 'dir')
        mkdir(ResultantFolder_Report);
    end
    set( handles.ResultantFolderEdit, 'String', ResultantFolder_Report);
end


% --- Executes on button press in ClusterReport_Button.
function ClusterReport_Button_Callback(hObject, eventdata, handles)
% hObject    handle to ClusterReport_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global ClusterImgPathList;
global AtlasPath_Report;
...
global AtlasFlag_Report;
global ResultantFolder_Report;
AtlasInfo_File = load(AtlasInfo_Report);
FieldName = fieldnames(AtlasInfo_File);
AtlasNameInfo = AtlasInfo_File.(FieldName{1});
ResultantFolder_Report_New = [ResultantFolder_Report filesep 'Report'];
mkdir(ResultantFolder_Report_New);
for i = 1:length(ClusterImgPathList)
    [~, ClusterImgFileName, Suffix] = fileparts(ClusterImgPathList{i});
    if strcmp(Suffix, '.gz')
        ClusterImgFileName = ClusterImgFileName(1 : end - 4);
    end
    NewClusterImg = [ResultantFolder_Report_New filesep ClusterImgFileName '_abs.nii'];
    cmd_PANDA = ['fslmaths ' ClusterImgPathList{i} ' -abs ' NewClusterImg ' && fslchfiletype NIFTI ' NewClusterImg '.gz'];
    system(cmd_PANDA);
    tmp_PANDA = load_nii(NewClusterImg);
    MinValue = min(min(min(tmp_PANDA.img(find(tmp_PANDA.img)))));
    MinThres = MinValue - 1.0e-20;
    ClusterIndexImg = [ResultantFolder_Report_New filesep ClusterImgFileName '_Index'];
    cmd_PANDA = ['cluster -i ' NewClusterImg ' -t ' num2str(MinThres) ' -o ' ClusterIndexImg];
    system(cmd_PANDA);
    cmd_PANDA = ['fslchfiletype NIFTI ' ClusterIndexImg];
    system(cmd_PANDA);
    ClusterIndexImg = [ClusterIndexImg '.nii'];
    tmp_PANDA = load_nii(ClusterIndexImg);
    ClusterIndex = unique(tmp_PANDA.img);
    ClusterIndex = ClusterIndex(2:end);
    
    % Resample the atlas according to the image
    Atlas_Resample = [ResultantFolder_Report_New filesep 'Atlas_tmp.nii.gz'];
    cmd_PANDA = ['applywarp -i ' AtlasPath_Report ' -r ' ClusterImgPathList{i} ' -o ' Atlas_Resample ' --interp=nn'];
    system(cmd_PANDA);
    
    RowQuantity = 0;
    for j = length(ClusterIndex):-1:1;
        disp(['Processing cluster ' num2str(ClusterIndex(j))]);
        NewClusterImg_SubMask{j} = [ResultantFolder_Report_New filesep ClusterImgFileName '_Mask_' num2str(ClusterIndex(j))];
        cmd_PANDA = ['fslmaths ' ClusterIndexImg ' -thr ' num2str(ClusterIndex(j)) ' -uthr ' num2str(ClusterIndex(j)) ' -bin ' NewClusterImg_SubMask{j} ...
            ' && fslchfiletype NIFTI ' NewClusterImg_SubMask{j}];
        system(cmd_PANDA);
        
        tmp_PANDA = load_nii([NewClusterImg_SubMask{j} '.nii']);
        Sub_AtlasMask = tmp_PANDA.img(find(tmp_PANDA.img));
        Sub_WholeVoxelSize = length(Sub_AtlasMask);
        Array_data_cell{RowQuantity + 1, 1} = num2str(length(ClusterIndex) - j + 1);
        Array_data_cell{RowQuantity + 1, 2} = Sub_WholeVoxelSize;
        
        NewClusterImg_SubAtlas{j} = [ResultantFolder_Report_New filesep ClusterImgFileName '_Atlas_' num2str(ClusterIndex(j))];
        cmd_PANDA = ['fslmaths ' NewClusterImg_SubMask{j} ' -mul ' Atlas_Resample ' ' NewClusterImg_SubAtlas{j}];
        system(cmd_PANDA);
        cmd_PANDA = ['fslchfiletype NIFTI ' NewClusterImg_SubAtlas{j}];
        system(cmd_PANDA);
        NewClusterImg_SubAtlas{j} = [NewClusterImg_SubAtlas{j} '.nii'];
        tmp_PANDA = load_nii(NewClusterImg_SubAtlas{j});
        SubCluster_data = tmp_PANDA.img(find(tmp_PANDA.img));
        ROIIndex = unique(SubCluster_data);
        
        for k = 1:length(ROIIndex)
            ROI_size(k) = length(find(SubCluster_data == ROIIndex(k)));
        end
        [~, OriginIndex] = sort(ROI_size, 2, 'descend');
        
        if length(ROIIndex)
            for k = 1:length(ROIIndex)
                if AtlasFlag_Report
                    Array_data_cell{RowQuantity + 1, 3} = AtlasNameInfo{ROIIndex(OriginIndex(k))};
                else
                    Array_data_cell{RowQuantity + 1, 3} = ['Area ' num2str(ROIIndex(OriginIndex(k)))];
                end
                Array_data_cell{RowQuantity + 1, 4} = length(find(SubCluster_data == ROIIndex(OriginIndex(k))));
                RowQuantity = RowQuantity + 1;
            end
        else
            Array_data_cell{RowQuantity + 1, 3} = 'Not in the atlas';
            RowQuantity = RowQuantity + 1;
        end
        
        delete([NewClusterImg_SubMask{j} '.nii']);
        delete(NewClusterImg_SubAtlas{j});
        ROI_size = 0;
    end
    
    delete(NewClusterImg);
    delete(ClusterIndexImg);
    delete(Atlas_Resample);
    % Write the information to the excel
    ResultantExcel = [ResultantFolder_Report_New filesep ClusterImgFileName '_Report.xls'];
    Cell_up = {'Index', 'voxel size in total', 'region name', 'voxel size'};
    Cell_left = [];
    if exist(ResultantExcel, 'file')
        delete(ResultantExcel);
    end
    g_xlswrite(Cell_left, Cell_up, Array_data_cell, ResultantExcel);
    display('Finished!');
    Array_data_cell = [];
end
...
%     set(handles.AtlasCheckBox, 'Value', 1);
    ...
    set(handles.Atlas_Edit, 'Enable', 'off');
    set(handles.Atlas_Button, 'Enable', 'off');
    set(handles.AtlasMenu, 'Enable', 'on');
    Menu_Index = get(handles.AtlasMenu, 'Value');
    AtlasPath_Report = AtlasPathMenu{Menu_Index};
    AtlasInfo_Report = AtlasInfoMenu{Menu_Index};
else
    AtlasFlag_Report = 0;
    PANDAPath = which('PANDA');
    [PANDAFolder, ~, ~] = fileparts(PANDAPath);
    AtlasPath_Report = [PANDAFolder filesep 'data/atlases/rICBM_DTI/rICBM_DTI_81_WMPM_FMRIB58.nii.gz'];
    set(handles.Atlas_Edit, 'String', AtlasPath_Report);
    set(handles.AtlasMenu, 'Enable', 'off');
    set(handles.Atlas_Edit, 'Enable', 'on');
    set(handles.Atlas_Button, 'Enable', 'on');
end


% --- Executes on selection change in AtlasMenu.
function AtlasMenu_Callback(hObject, eventdata, handles)
% hObject    handle to AtlasMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns AtlasMenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from AtlasMenu
global AtlasPath_Report;
global AtlasInfo_Report;
global AtlasPathMenu;
global AtlasInfoMenu;
AtlasIndex = get(hObject, 'Value');
AtlasPath_Report = AtlasPathMenu{AtlasIndex};
AtlasInfo_Report = AtlasInfoMenu{AtlasIndex};


% --- Executes during object creation, after setting all properties.
function AtlasMenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to AtlasMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes when PANDAClusterLocaterFigure is resized.
function PANDAClusterLocaterFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDAClusterLocaterFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    ResizeClusterImageTable(handles);
end


function ResizeClusterImageTable(handles)
ClusterImage = get(handles.ClusterImgTable, 'data');
PositionFigure = get(handles.PANDAClusterLocaterFigure, 'Position');
WidthCell{1} = PositionFigure(3);
WidthCell{2} = 0;
if ~isempty(ClusterImage)
    for i = 1:length(ClusterImage)
        tmp_PANDA{i} = length(ClusterImage{i}) * 8;
