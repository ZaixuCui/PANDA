function varargout = PANDA_MergeVector(varargin)
% GUI for merging vector (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_MERGEVECTOR MATLAB code for PANDA_MergeVector.fig
%      PANDA_MERGEVECTOR, by itself, creates a new PANDA_MERGEVECTOR or raises the existing
%      singleton*.
%
%      H = PANDA_MERGEVECTOR returns the handle to a new PANDA_MERGEVECTOR or the handle to
%      the existing singleton*.
%
%      PANDA_MERGEVECTOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_MERGEVECTOR.M with the given input arguments.
%
%      PANDA_MERGEVECTOR('Property','Value',...) creates a new PANDA_MERGEVECTOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_MergeVector_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_MergeVector_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_MergeVector

% Last Modified by GUIDE v2.5 16-Jul-2013 11:10:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_MergeVector_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_MergeVector_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_MergeVector is made visible.
function PANDA_MergeVector_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_MergeVector (see VARARGIN)

% Choose default command line output for PANDA_MergeVector
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_MergeVector wait for user response (see UIRESUME)
...
WidthCell{1} = 450;
WidthCell{2} = 0;
set( handles.VectorFileTable, 'ColumnWidth', WidthCell);


% --- Outputs from this function are returned to the command line.
function varargout = PANDA_MergeVector_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in VectorFileButton.
function VectorFileButton_Callback(hObject, eventdata, handles)
% hObject    handle to VectorFileButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global RawVectorList;
RawVectorList_Button = get(hObject, 'UserData');
[x, RawVectorList, Done] = PANDA_Select('file', RawVectorList_Button);
if Done == 1
    set( hObject, 'UserData', RawVectorList );
    if ~isempty(RawVectorList)
        RawVectorTable = RawVectorList;
        set( handles.VectorFileTable, 'data', RawVectorTable);
    else
...
% set(handles.VectorFileButton, 'Enable', 'off');
...
tmp_PANDA = load(RawVectorList{1});
[RowsQuantity ColumnsQuantity] = size(tmp_PANDA);
if RowsQuantity ~= 1
    MergedMatrix(1, :) = tmp_PANDA';
else
    MergedMatrix(1, :) = tmp_PANDA;
end
for i = 2:length(RawVectorList)
    tmp_PANDA = load(RawVectorList{i});
    [RowsQuantity ColumnsQuantity] = size(tmp_PANDA);
    if RowsQuantity ~= 1
        MergedMatrix(i, :) = tmp_PANDA';
    else
        MergedMatrix(i, :) = tmp_PANDA;
    end
end
save([ResultantFolder filesep 'MergedMatrix.mat'], 'MergedMatrix');
disp('Done!');

set(handles.VectorFileButton, 'Enable', 'on');
