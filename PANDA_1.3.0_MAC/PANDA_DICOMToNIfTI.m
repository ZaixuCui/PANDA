function varargout = PANDA_DICOMToNIfTI(varargin)
% GUI for converting DICOM to NIfTI (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_DICOMTONIFTI MATLAB code for PANDA_DICOMToNIfTI.fig
%      PANDA_DICOMTONIFTI, by itself, creates a new PANDA_DICOMTONIFTI or raises the existing
%      singleton*.
%
%      H = PANDA_DICOMTONIFTI returns the handle to a new PANDA_DICOMTONIFTI or the handle to
%      the existing singleton*.
%
%      PANDA_DICOMTONIFTI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_DICOMTONIFTI.M with the given input arguments.
%
%      PANDA_DICOMTONIFTI('Property','Value',...) creates a new PANDA_DICOMTONIFTI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_DICOMToNIfTI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_DICOMToNIfTI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_DICOMToNIfTI

% Last Modified by GUIDE v2.5 01-Oct-2012 10:00:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_DICOMToNIfTI_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_DICOMToNIfTI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_DICOMToNIfTI is made visible.
function PANDA_DICOMToNIfTI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_DICOMToNIfTI (see VARARGIN)

% Choose default command line output for PANDA_DICOMToNIfTI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_DICOMToNIfTI wait for user response (see UIRESUME)
% uiwait(handles.PANDADICOMNIfTIFigure);
global DICOMNIfTI_Data_Raw_Path_Cell;
global DICOMNIfTI_SubjectIDArray;
global DICOMNIfTI_DestinationPath_Edit;
global DICOMNIfTI_TensorPrefixEdit;
global DICOMNIfTI_Pipeline_opt;
global PANDAPath;
global DICOMToNIfTIType;

[PANDAPath, y, z] = fileparts(which('PANDA.m'));

DICOMToNIfTIType = 'T1';

DICOMNIfTI_Data_Raw_Path_Cell = '';
% Pipeline options
DICOMNIfTI_Pipeline_opt.flag_verbose = 0;
DICOMNIfTI_Pipeline_opt.flag_pause = 0;
set( handles.batchRadio, 'Value', 1);
DICOMNIfTI_Pipeline_opt.mode = 'background';
set( handles.QsubOptionsEdit, 'String', '');
set( handles.QsubOptionsEdit, 'Enable', 'off');
DICOMNIfTI_Pipeline_opt.qsub_options = '';
...
if ~isempty(QuantityOfCpu)
    DICOMNIfTI_Pipeline_opt.max_queued = str2num(QuantityOfCpu);
else
    DICOMNIfTI_Pipeline_opt.max_queued = 2;
end
set(handles.MaxQueuedEdit, 'string', num2str(DICOMNIfTI_Pipeline_opt.max_queued));
DICOMNIfTI_Pipeline_opt.path_logs = [DICOMNIfTI_DestinationPath_Edit filesep 'logs'];
...
SetIcon(handles);
%
TipStr = sprintf('Input all the folders containg DICOM files.');
...
function varargout = PANDA_DICOMToNIfTI_OutputFcn(hObject, eventdata, handles) 
...
global DICOMNIfTI_DestinationPath_Edit;
global DICOMNIfTI_TensorPrefixEdit;
DICOMNIfTI_Data_Raw_Path_Cell_Origin = DICOMNIfTI_Data_Raw_Path_Cell;
DataRawPathCell_Button = get(hObject, 'UserData');
[a, DICOMNIfTI_Data_Raw_Path_Cell, Done] = PANDA_Select('dir', DataRawPathCell_Button);
if Done == 1
    set(hObject, 'UserData', DICOMNIfTI_Data_Raw_Path_Cell);
    if ~isempty(DICOMNIfTI_Data_Raw_Path_Cell)
        DICOMNIfTI_SubjectIDArray = '';
        DICOMNIfTI_DestinationPath_Edit = '';
        DICOMNIfTI_TensorPrefixEdit = '';
        set( handles.DestinationPathEdit, 'String', '' );
        set( handles.SubjectIDEdit, 'String', '' );
        set( handles.TensorPrefixEdit, 'String', '');
        set( handles.RawPath_DestPath, 'data', DICOMNIfTI_Data_Raw_Path_Cell );
        ResizeRawDestTable(handles)
    else
        DICOMNIfTI_Data_Raw_Path_Cell = DICOMNIfTI_Data_Raw_Path_Cell_Origin;
    end
end
...
global DICOMNIfTI_SubjectIDArray;
global DICOMNIfTI_Data_Raw_Path_Cell;
SubjectID_Text = mat2str(DICOMNIfTI_SubjectIDArray);
SubjectID_Text_New = get( hObject, 'string' );
if ~isempty(DICOMNIfTI_SubjectIDArray) & ~strcmp(SubjectID_Text, SubjectID_Text_New)
...
if isempty(DICOMNIfTI_SubjectIDArray)
    SubjectID_Text = SubjectID_Text_New;
end
if ~isempty(SubjectID_Text)
    % Pick the destion path from edit text
    % User can input without selecting from DirSelect GUI
    DICOMNIfTI_DestinationPath_Edit = get( handles.DestinationPathEdit, 'string' );
    if ~isempty(DICOMNIfTI_DestinationPath_Edit)
        try
            DICOMNIfTI_SubjectIDArray = eval( SubjectID_Text );
            set( hObject, 'string', mat2str(DICOMNIfTI_SubjectIDArray) );
            % Judge whther the quantity of the subjects is equal to the quantity of the
            % subjects id
            if length(DICOMNIfTI_Data_Raw_Path_Cell) ~= length(DICOMNIfTI_SubjectIDArray)
                info{1} = ['Your subject IDs are ' mat2str(DICOMNIfTI_SubjectIDArray)];
                ...
                DestiantionPath_Subject = cell(length(DICOMNIfTI_SubjectIDArray),1);
                for i = 1:length(DICOMNIfTI_SubjectIDArray)
                    DestiantionPath_Subject{i} = [DICOMNIfTI_DestinationPath_Edit filesep num2str(DICOMNIfTI_SubjectIDArray(i),'%05.0f')];
                end
                % Display the raw data path and the destination path in the table
                %save x.mat Data_Raw_Path_Cell DestiantionPath_Subject
                RawData_Destination = [DICOMNIfTI_Data_Raw_Path_Cell, DestiantionPath_Subject];
...
DICOMNIfTI_DestinationPath_Edit_New = get( hObject, 'string' );
if ~isempty(DICOMNIfTI_DestinationPath_Edit) & ~strcmp(DICOMNIfTI_DestinationPath_Edit, DICOMNIfTI_DestinationPath_Edit_New)
    button = questdlg('Destination path is changed, are you sure ?','Sure to change ?','Yes','No','Yes');
    switch button
    case 'Yes'
        DICOMNIfTI_DestinationPath_Edit = DICOMNIfTI_DestinationPath_Edit_New;
        DICOMNIfTI_Pipeline_opt.path_logs = [DICOMNIfTI_DestinationPath_Edit filesep 'logs'];
        if ~isempty(DICOMNIfTI_SubjectIDArray)
            % Combine Destination path to Subject id 
            DestiantionPath_Subject = cell(length(DICOMNIfTI_SubjectIDArray),1);
    ...
    case 'No'
        set(hObject, 'string', DICOMNIfTI_DestinationPath_Edit);
        return;
    end
elseif isempty(DICOMNIfTI_DestinationPath_Edit)
    DICOMNIfTI_DestinationPath_Edit = DICOMNIfTI_DestinationPath_Edit_New;
    DICOMNIfTI_Pipeline_opt.path_logs = [DICOMNIfTI_DestinationPath_Edit filesep 'logs'];
        if ~isempty(DICOMNIfTI_SubjectIDArray)
            % Combine Destination path to Subject id 
...
if ~isempty(DICOMNIfTI_DestinationPath_Edit)
    LogPath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE.lock'];
    if exist(LogPath, 'file')
        ...
        switch button
        case 'Yes'
            system(['rm ' DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE.lock']);
        case 'No'
            system(['rm -R ' DICOMNIfTI_DestinationPath_Edit filesep]);
            mkdir(DICOMNIfTI_DestinationPath_Edit);
            return;
        end
    end
end
...
if DestinationPath_Select ~= 0
    ...
    DICOMNIfTI_DestinationPath_Edit = DestinationPath_Select;
    DICOMNIfTI_Pipeline_opt.path_logs = [DICOMNIfTI_DestinationPath_Edit filesep 'logs'];
    LogPath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE.lock'];
    if exist(LogPath, 'file')
        info{1} = 'A lock of pipeline is already exist in this result path!';
...
DICOMNIfTI_TensorPrefixEdit_New = get(hObject, 'string');
ChangeFlag = 0;
if ~isempty(DICOMNIfTI_TensorPrefixEdit) & ~strcmp(DICOMNIfTI_TensorPrefixEdit, DICOMNIfTI_TensorPrefixEdit_New)
    button = questdlg('File prefix is changed, are you sure ?','Sure to change ?','Yes','No','Yes');
    switch button
    case 'Yes'
        DICOMNIfTI_TensorPrefixEdit = DICOMNIfTI_TensorPrefixEdit_New;
    case 'No'
        set(hObject, 'string', DICOMNIfTI_TensorPrefixEdit);
        return;
    end
end
if isempty(DICOMNIfTI_TensorPrefixEdit)
    DICOMNIfTI_TensorPrefixEdit = DICOMNIfTI_TensorPrefixEdit_New;
end
...
global DICOMNIfTI_Pipeline_opt;
DICOMNIfTI_Pipeline_opt.max_queued = str2num(get(hObject, 'String'));
...
global DICOMNIfTI_Pipeline_opt;
DICOMNIfTI_Pipeline_opt.qsub_options = get(hObject, 'String');
...
global DICOMNIfTI_TensorPrefixEdit;
global DICOMNIfTI_Pipeline_opt;
global DICOMNIfTI_opt;
global JobStatusMonitorTimerDICOMNIfTI;
global DICOMNIfTIStopFlag;
global LockExistDICOMNIfTI;
global LockDisappearDICOMNIfTI;
global PANDAPath;
global DICOMToNIfTIType;

DICOMNIfTIStopFlag = '';
LockFilePath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE.lock'];
if exist(LockFilePath, 'file') | strcmp(get(handles.RawDataPathButton, 'Enable'), 'off')
    ...
    elseif isempty(DICOMNIfTI_SubjectIDArray)
        msgbox('Please assign IDs for subjects!');
    else
        LogPathPermissionDenied = 0;
        try
            if ~exist(DICOMNIfTI_DestinationPath_Edit, 'dir')
                mkdir(DICOMNIfTI_DestinationPath_Edit);
            end
            if ~exist([DICOMNIfTI_DestinationPath_Edit filesep 'logs'], 'dir')
                mkdir([DICOMNIfTI_DestinationPath_Edit filesep 'logs']);
            end
            x = 1;
            save([DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'permission_tag.mat'], 'x');
        catch
        ...
        if ~LogPathPermissionDenied
            ...
            if strcmp(button, 'Yes')
                ...
                ParameterSaveFilePath = [DICOMNIfTI_DestinationPath_Edit  filesep DateNowString '.PANDA_DICOMNIfTI'];
                cmdString = [ 'save ' ParameterSaveFilePath ' DICOMNIfTI_Data_Raw_Path_Cell' ' DICOMNIfTI_DestinationPath_Edit' ' DICOMNIfTI_SubjectIDArray' ...
                        ' DICOMNIfTI_TensorPrefixEdit' ' DICOMNIfTI_Pipeline_opt' ' PANDAPath'];
                eval(cmdString);
                ...
                if exist( ParameterSaveFilePath, 'file' )
                    ...
                    DICOMNIfTI_Pipeline_opt.mode_pipeline_manager = 'background';
                    DICOMNIfTI_Pipeline_opt.flag_verbose = 0;
                    DICOMNIfTI_Pipeline_opt.flag_pause = 0;
                    
                    for i = 1:length(DICOMNIfTI_Data_Raw_Path_Cell)
                        
                        Number_Of_Subject_String = num2str(DICOMNIfTI_SubjectIDArray(i), '%05d');
                        SubjectFolder = [DICOMNIfTI_DestinationPath_Edit filesep Number_Of_Subject_String];
                        
                        % Save the raw path and the path of nii data to matrix file
                        variable_name = ['subject_info_',Number_Of_Subject_String];
                        Subject_Info_File = [DICOMNIfTI_DestinationPath_Edit filesep 'subject_info.mat'];
                        tmp_PANDA = [DICOMNIfTI_Data_Raw_Path_Cell{i} '  ' SubjectFolder '/'];
                        ...
                        fid = fopen([DICOMNIfTI_DestinationPath_Edit filesep 'subject_info.txt'], 'a+');
                        ...
                        while(~feof(fid))
                            ...
                            if(strcmp(raw_path, DICOMNIfTI_Data_Raw_Path_Cell{i}))
                                exist_flag = 1;
                                break;
                            end
                        end
                        if exist_flag == 0
                            fprintf(fid, DICOMNIfTI_Data_Raw_Path_Cell{i});fprintf(fid, ' \t');
                            fprintf(fid, [DICOMNIfTI_DestinationPath_Edit filesep Number_Of_Subject_String '/']);fprintf(fid, '\n');
                        end
                        fclose(fid);
   
                        JobName = ['dcm2nii_' num2str(DICOMNIfTI_SubjectIDArray(i), '%05d')];
                        if strcmp(DICOMToNIfTIType, 'T1')
                            pipeline.(JobName).command = 'g_dcm2nii_t1(opt.DICOMFolder,opt.Prefix,opt.OutputFolder)';
                        elseif strcmp(DICOMToNIfTIType, 'dMRI')
                        ...
                        pipeline.(JobName).opt.DICOMFolder = DICOMNIfTI_Data_Raw_Path_Cell{i};
                        if ~isempty(DICOMNIfTI_TensorPrefixEdit)
                            pipeline.(JobName).opt.Prefix = [DICOMNIfTI_TensorPrefixEdit '_' Number_Of_Subject_String];
                        else
                            pipeline.(JobName).opt.Prefix = Number_Of_Subject_String;
                        end
                    ...
                    psom_run_pipeline(pipeline, DICOMNIfTI_Pipeline_opt);
                    ...
                    set(handles.T1Radio, 'Enable', 'off');
                    set(handles.dMRIRadio, 'Enable', 'off');
                    set(handles.fMRIRadio, 'Enable', 'off');
                    ...
                    StatusFilePath = [DICOMNIfTI_Pipeline_opt.path_logs filesep 'PIPE_status.mat'];

                    JobsStatus = cell(length(DICOMNIfTI_Data_Raw_Path_Cell), 4);
                    for i = 1:length(DICOMNIfTI_Data_Raw_Path_Cell)
                        JobsStatus{i, 1} = 'wait';
                        JobsStatus{i, 2} = 'wait';
                        JobsStatus{i, 3} = 'wait';
                    ...
                    LockExistDICOMNIfTI = 0;
                    LockDisappearDICOMNIfTI = 0;
                    % Start monitor function
                    DICOMNIfTIStopFlag = '';
                    JobStatusMonitorTimerDICOMNIfTI = timer( 'TimerFcn', {@JobStatusMonitorDICOMNIfTI, handles}, 'period', 10, 'ExecutionMode', 'fixedRate');
                    start(JobStatusMonitorTimerDICOMNIfTI);
                else
                    return;
                end
            end
...
if strcmp(get(handles.RawDataPathButton, 'Enable'), 'off')
    ...
    [ParameterSaveFileName,ParameterSaveFilePath] = uigetfile({'*.PANDA_DICOMNIfTI','DICOM->NIfTI-files (*.PANDA_DICOMNIfTI)'},'Load Configuration');
    if ParameterSaveFileName ~= 0
        ...
        eval( cmdString );
        set( handles.DestinationPathEdit, 'String', DICOMNIfTI_DestinationPath_Edit);
        set( handles.SubjectIDEdit, 'String', mat2str(DICOMNIfTI_SubjectIDArray));
        set( handles.TensorPrefixEdit, 'String', DICOMNIfTI_TensorPrefixEdit);
        if strcmp(DICOMToNIfTIType, 'T1')
            set(handles.T1Radio, 'Value', 1);
        elseif strcmp(DICOMToNIfTIType, 'dMRI')
            set(handles.dMRIRadio, 'Value', 1);
        else
            set(handles.fMRIRadio, 'Value', 1);
        end
        
        for i = 1:length(DICOMNIfTI_Data_Raw_Path_Cell)
            DestinationPathCell{i} = [DICOMNIfTI_DestinationPath_Edit filesep num2str(DICOMNIfTI_SubjectIDArray(i), '%05d')];
        end
        DestinationPathCell = reshape(DestinationPathCell, length(DestinationPathCell), 1);
        RawPath_DestPathCell = [DICOMNIfTI_Data_Raw_Path_Cell, DestinationPathCell];
        set( handles.RawPath_DestPath, 'data', RawPath_DestPathCell );
        ResizeRawDestTable(handles);
        if strcmp(DICOMNIfTI_Pipeline_opt.mode,'background')
            set( handles.batchRadio, 'Value', 1 );
        else
            set( handles.qsubRadio, 'Value', 1 );
            set( handles.QsubOptionsEdit, 'Enable', 'on');
            set( handles.QsubOptionsEdit, 'string', DICOMNIfTI_Pipeline_opt.qsub_options);
        end
        set( handles.MaxQueuedEdit, 'string', num2str(DICOMNIfTI_Pipeline_opt.max_queued) );
        % Display job status in the Job Status Table
        LockFilePath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE.lock'];
        if exist(LockFilePath, 'file')
            LockExistDICOMNIfTI = 1;
            LockDisappearDICOMNIfTI = 0;
            JobStatusMonitorTimerDICOMNIfTI = timer( 'TimerFcn', {@JobStatusMonitorDICOMNIfTI, handles}, 'period', 10, 'ExecutionMode', 'fixedRate');
            start(JobStatusMonitorTimerDICOMNIfTI);
            ...
            JobStatusMonitorDICOMNIfTI(hObject, eventdata, handles);
        end
    end
end
...
switch button
    case 'Yes'
        % Stop the monitor
        if ~isempty(JobStatusMonitorTimerDICOMNIfTI)
            stop(JobStatusMonitorTimerDICOMNIfTI);
            clear global JobStatusMonitorTimerDICOMNIfTI;
        end
        
        clear global LockExistDICOMNIfTI;
        clear global LockDisappearDICOMNIfTI;
        ...
        set(handles.TensorPrefixEdit, 'String', '');
        set(handles.T1Radio, 'Enable', 'on');
        set(handles.dMRIRadio, 'Enable', 'on');
        set(handles.fMRIRadio, 'Enable', 'on');
...
LockFilePath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE.lock'];
if isempty(DICOMNIfTI_DestinationPath_Edit) || ~exist(LockFilePath, 'file')
...
% --- Executes when PANDADICOMNIfTIFigure is resized.
function PANDADICOMNIfTIFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDADICOMNIfTIFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    PositionFigure = get(handles.PANDADICOMNIfTIFigure, 'Position');
    ResizeRawDestTable(handles);
    ResizeJobStatusTable(handles);
    ...
    set( handles.TypeUipanel, 'FontSize', FontSizePipelineOptionsUipanel );
end
...
switch get(hObject, 'tag')
    case 'batchRadio'
        ...
        set(handles.MaxQueuedEdit, 'string', num2str(DICOMNIfTI_Pipeline_opt.max_queued));
    case 'qsubRadio'
        DICOMNIfTI_Pipeline_opt.mode = 'qsub';
        DICOMNIfTI_Pipeline_opt.qsub_options = '-V -q all.q';
        set( handles.QsubOptionsEdit, 'Enable', 'on');
        set( handles.QsubOptionsEdit, 'String', '-V -q all.q');
        DICOMNIfTI_Pipeline_opt.max_queued = 40;
        set(handles.MaxQueuedEdit, 'string', num2str(DICOMNIfTI_Pipeline_opt.max_queued));
end


% --- Executes when user attempts to close PANDADICOMNIfTIFigure.
function PANDADICOMNIfTIFigure_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to PANDADICOMNIfTIFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
switch button
    case 'Yes'
        ...
        clear global DICOMNIfTI_Data_Raw_Path_Cell;
        clear global DICOMNIfTI_DestinationPath_Edit;
        clear global DICOMNIfTI_SubjectIDArray;
        clear global DICOMNIfTI_TensorPrefixEdit;
        clear global DICOMNIfTI_Pipeline_opt;
        clear global DICOMNIfTI_opt;
        clear global DICOMNIfTIStopFlag;
        clear global LockExistDICOMNIfTI;
        clear global LockDisappearDICOMNIfTI;
        delete(hObject);
    case 'No'
...
function JobStatusMonitorDICOMNIfTI(hObject, eventdata, handles)
...
LockFilePath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE.lock'];
if ~LockExistDICOMNIfTI & exist( LockFilePath, 'file' )
    LockExistDICOMNIfTI = 1;
end
if LockExistDICOMNIfTI & ~exist( LockFilePath, 'file' )
    LockDisappearDICOMNIfTI = 1;
end
if LockDisappearDICOMNIfTI == 1
    % Set edit box enable
    set(handles.RawDataPathButton, 'Enable', 'on');
    set(handles.DestinationPathEdit, 'Enable', 'on');
    set(handles.DestinationPathButton, 'Enable', 'on');
    ...
    set(handles.qsubRadio, 'Enable', 'on');
    set(handles.MaxQueuedEdit, 'Enable', 'on');
    if strcmp(DICOMNIfTI_Pipeline_opt.mode,'qsub')
        set(handles.QsubOptionsEdit, 'Enable', 'on');
    end
    % Stop Monitor
    if ~isempty(JobStatusMonitorTimerDICOMNIfTI)
...
StatusFilePath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'PIPE_status_backup.mat'];
if exist( StatusFilePath, 'file' )
    warning('off');
    try    
        cmdString = ['load ' StatusFilePath];
        eval(cmdString);
        SubjectQuantity = length(DICOMNIfTI_Data_Raw_Path_Cell);
        JobName = {'dcm2nii'};
        ...
        % Calculate job status of the ith subject 
            ...
            JobLeft = 0;
            StatusArray{i} = 'none';
            SubjectIDArrayString{i} = num2str(DICOMNIfTI_SubjectIDArray(i),'%05d');
        ...
        % 
        JobsFinishFilePath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs' filesep 'jobs.finish'];
...
% --- Executes when selected object is changed in TypeUipanel.
function TypeUipanel_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in TypeUipanel 
% eventdata  structure with the following fields (see UIBUTTONGROUP)
%	EventName: string 'SelectionChanged' (read only)
%	OldValue: handle of the previously selected object or empty if none was selected
%	NewValue: handle of the currently selected object
...
switch get(hObject, 'tag')
    case 'T1Radio'
        DICOMToNIfTIType = 'T1';
    case 'dMRIRadio'
        DICOMToNIfTIType = 'dMRI';
    case 'fMRIRadio'
        DICOMToNIfTIType = 'fMRI';
end
...
LogsPath = '';
if isempty(DICOMNIfTI_DestinationPath_Edit)
    [ParameterSaveFileName,ParameterSaveFilePath] = uigetfile({'*.PANDA_DICOMNIfTI','PANDA-files (*.PANDA_DICOMNIfTI)'},'Load Configuration');
    if ParameterSaveFileName ~= 0
        cmdString = ['PANDAConfiguration = load(''' ParameterSaveFilePath filesep ParameterSaveFileName ''', ''-mat'')'];
        eval( cmdString );
        LogsPath = [PANDAConfiguration.DICOMNIfTI_DestinationPath_Edit filesep 'logs'];    
    end
else
    LogsPath = [DICOMNIfTI_DestinationPath_Edit filesep 'logs'];
end
if ~isempty(LogsPath)
    try
