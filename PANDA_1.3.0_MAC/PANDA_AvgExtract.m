function varargout = PANDA_AvgExtract(varargin)
% GUI for extracting average value (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_AVGEXTRACT MATLAB code for PANDA_AvgExtract.fig
%      PANDA_AVGEXTRACT, by itself, creates a new PANDA_AVGEXTRACT or raises the existing
%      singleton*.
%
%      H = PANDA_AVGEXTRACT returns the handle to a new PANDA_AVGEXTRACT or the handle to
%      the existing singleton*.
%
%      PANDA_AVGEXTRACT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_AVGEXTRACT.M with the given input arguments.
%
%      PANDA_AVGEXTRACT('Property','Value',...) creates a new PANDA_AVGEXTRACT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_AvgExtract_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_AvgExtract_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_AvgExtract

% Last Modified by GUIDE v2.5 22-Jul-2015 21:47:54

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_AvgExtract_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_AvgExtract_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_AvgExtract is made visible.
function PANDA_AvgExtract_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_AvgExtract (see VARARGIN)

% Choose default command line output for PANDA_AvgExtract
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_AvgExtract wait for user response (see UIRESUME)
% uiwait(handles.PANDAAvgExtractFigure);
% Set icon
SetIcon(handles);
%
...
WidthCell{1} = 450;
WidthCell{2} = 0;
set( handles.ImagesListTable, 'ColumnWidth', WidthCell);


% --- Outputs from this function are returned to the command line.
function varargout = PANDA_AvgExtract_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in ImagesListButton.
function ImagesListButton_Callback(hObject, eventdata, handles)
% hObject    handle to ImagesListButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global AvgExtract_ImagesList;
ImagesList_Button = get(hObject, 'UserData');
[~, AvgExtract_ImagesList, Done] = PANDA_Select('file', ImagesList_Button);
if Done == 1
    set( hObject, 'UserData', AvgExtract_ImagesList );
    if ~isempty(AvgExtract_ImagesList)
        set( handles.ImagesListTable, 'data', AvgExtract_ImagesList);
    else
        ROIIndexTable = cell(4,1);
        set( handles.ImagesListTable, 'data', ROIIndexTable);
    end
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global AvgExtract_ClusterImage;
[ClusterImage_FolderName ParentFolder] = uigetfile({'*.nii.gz','NIfTI-files (*.nii.gz)'});
ClusterImage_FolderPath = [ParentFolder ClusterImage_FolderName];
if ~isnumeric(ClusterImage_FolderPath)
    set( handles.ClusterImage_Edit, 'string', ClusterImage_FolderPath );
    AvgExtract_ClusterImage = ClusterImage_FolderPath;
end


function ClusterImage_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to ClusterImage_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ClusterImage_Edit as text
%        str2double(get(hObject,'String')) returns contents of ClusterImage_Edit as a double
global AvgExtract_ClusterImage;
set(hObject, 'String', AvgExtract_ClusterImage);


% --- Executes during object creation, after setting all properties.
function ClusterImage_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ClusterImage_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
function ResultantFolder_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to ResultantFolder_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ResultantFolder_Edit as text
%        str2double(get(hObject,'String')) returns contents of ResultantFolder_Edit as a double
global AvgExtract_ResultantFolder;
ResultantFolder = get(hObject, 'String');
if ~exist(ResultantFolder, 'dir')
    try
        mkdir(ResultantFolder);
        AvgExtract_ResultantFolder = ResultantFolder;
    catch
        msgbox('The path you input is illegal !');
        set(hObject, 'String', '');
        AvgExtract_ResultantFolder = '';
    end
end


% --- Executes during object creation, after setting all properties.
function ResultantFolder_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ResultantFolder_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in ResultantFolder_Button.
function ResultantFolder_Button_Callback(hObject, eventdata, handles)
% hObject    handle to ResultantFolder_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global AvgExtract_ResultantFolder;
AvgExtract_ResultantFolder = uigetdir;
if AvgExtract_ResultantFolder
    if ~exist(AvgExtract_ResultantFolder, 'dir')
        mkdir(AvgExtract_ResultantFolder);
    end
    set( handles.ResultantFolder_Edit, 'String', AvgExtract_ResultantFolder);
end
...
global AvgExtract_ResultantFolder;
if isempty(AvgExtract_ImagesList)
    msgbox('Please input the images list for all subjects!');
elseif isempty(AvgExtract_ClusterImage)
    msgbox('Please input the cluster index image!');
elseif isempty(AvgExtract_ResultantFolder)
    msgbox('Please input the resultant folder!');
else
    if ~exist(AvgExtract_ResultantFolder)
        mkidr(AvgExtract_ResultantFolder);
    end
    ResultantFile = [AvgExtract_ResultantFolder filesep 'AvgExtract.xls'];
    if exist(ResultantFile, 'file')
        delete(ResultantFile);
    end
    TmpImg = [AvgExtract_ResultantFolder filesep 'tmp.nii'];
    PANDA_cmd = ['fslchfiletype NIFTI ' AvgExtract_ClusterImage ' ' TmpImg];
    system(PANDA_cmd);
    PANDA_data = load_nii(TmpImg);
    ClusterIndex = unique(PANDA_data.img);
    ClusterIndex = ClusterIndex(2:end);
    delete(TmpImg);
    
    % Resolution of cluster image
    disp('Check if the resolution is mathch!');
    [~, ClusterImage_dim1] = system(['fslval ' AvgExtract_ClusterImage ' dim1']);
    ClusterImage_dim1 = str2num(ClusterImage_dim1);
    [~, ClusterImage_dim2] = system(['fslval ' AvgExtract_ClusterImage ' dim2']);
    ClusterImage_dim2 = str2num(ClusterImage_dim2);
    [~, ClusterImage_dim3] = system(['fslval ' AvgExtract_ClusterImage ' dim3']);
    ClusterImage_dim3 = str2num(ClusterImage_dim3);
    [~, ClusterImage_pixdim1] = system(['fslval ' AvgExtract_ClusterImage ' pixdim1']);
    ClusterImage_pixdim1 = str2num(ClusterImage_pixdim1);
    [~, ClusterImage_pixdim2] = system(['fslval ' AvgExtract_ClusterImage ' pixdim2']);
    ClusterImage_pixdim2 = str2num(ClusterImage_pixdim2);
    [~, ClusterImage_pixdim3] = system(['fslval ' AvgExtract_ClusterImage ' pixdim3']);
    ClusterImage_pixdim3 = str2num(ClusterImage_pixdim3);
    
    for i = 1:length(AvgExtract_ImagesList)
        [~, SubjectImage_dim1] = system(['fslval ' AvgExtract_ImagesList{i} ' dim1']);
        SubjectImage_dim1 = str2num(SubjectImage_dim1);
        [~, SubjectImage_dim2] = system(['fslval ' AvgExtract_ImagesList{i} ' dim2']);
        SubjectImage_dim2 = str2num(SubjectImage_dim2);
        [~, SubjectImage_dim3] = system(['fslval ' AvgExtract_ImagesList{i} ' dim3']);
        SubjectImage_dim3 = str2num(SubjectImage_dim3);
        [~, SubjectImage_pixdim1] = system(['fslval ' AvgExtract_ImagesList{i} ' pixdim1']);
        SubjectImage_pixdim1 = str2num(SubjectImage_pixdim1);
        [~, SubjectImage_pixdim2] = system(['fslval ' AvgExtract_ImagesList{i} ' pixdim2']);
        SubjectImage_pixdim2 = str2num(SubjectImage_pixdim2);
        [~, SubjectImage_pixdim3] = system(['fslval ' AvgExtract_ImagesList{i} ' pixdim3']);
        SubjectImage_pixdim3 = str2num(SubjectImage_pixdim3);
        if ClusterImage_dim1 ~= SubjectImage_dim1 | ClusterImage_dim2 ~= SubjectImage_dim2 |...
              ClusterImage_dim3 ~= SubjectImage_dim3 | ClusterImage_pixdim1 ~= SubjectImage_pixdim1 |...
              ClusterImage_pixdim2 ~= SubjectImage_pixdim2 | ClusterImage_pixdim3 ~= SubjectImage_pixdim3
          PANDA_msg{1} = ['The resolution of ' AvgExtract_ImagesList{i} ' does not match the cluster image ' ...
              AvgExtract_ClusterImage '!'];
          PANDA_msg{2} = 'Please check!';
          msgbox(PANDA_msg);
          return;
        end
    end
    ...
    for i = 1:length(ClusterIndex)
        ...
        disp(['Processing cluster ' num2str(i) ': index=' num2str(ClusterIndex(i))]);
        Cluster_Index_Cell{i} = ['Cluster index=' num2str(ClusterIndex(i))];
        
        PANDA_cmd = ['fslmaths ' AvgExtract_ClusterImage ' -thr ' num2str(ClusterIndex(i)) ' -uthr ' ...
            num2str(ClusterIndex(i)) ' ' TmpImg];
        system(PANDA_cmd);
        for j = 1:length(AvgExtract_ImagesList)
            PANDA_cmd = ['fslstats ' AvgExtract_ImagesList{j} ' -k ' TmpImg ' -M'];
            [~, result] = system(PANDA_cmd);
            Array_Data{j, i} = str2num(result);
        end
    end
    delete([TmpImg '.gz']);
    g_xlswrite( AvgExtract_ImagesList, Cluster_Index_Cell, Array_Data, ResultantFile );
...
% --- Executes when entered data in editable cell(s) in ImagesListTable.
function ImagesListTable_CellEditCallback(hObject, eventdata, handles)
% hObject    handle to ImagesListTable (see GCBO)
...
% --- Executes when PANDAAvgExtractFigure is resized.
function PANDAAvgExtractFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDAAvgExtractFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    ResizeImageListTable(handles);
end


function ResizeImageListTable(handles)
ImageList = get(handles.ImagesListTable, 'data');
PositionFigure = get(handles.PANDAAvgExtractFigure, 'Position');
WidthCell{1} = PositionFigure(3);
WidthCell{2} = 0;
if ~isempty(ImageList)
    for i = 1:length(ImageList)
        tmp_PANDA{i} = length(ImageList{i}) * 8;
