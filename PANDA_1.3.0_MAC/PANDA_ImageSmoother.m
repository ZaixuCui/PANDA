function varargout = PANDA_ImageSmoother(varargin)
% PANDA_IMAGESMOOTHER MATLAB code for PANDA_ImageSmoother.fig
%      PANDA_IMAGESMOOTHER, by itself, creates a new PANDA_IMAGESMOOTHER or raises the existing
%      singleton*.
%
%      H = PANDA_IMAGESMOOTHER returns the handle to a new PANDA_IMAGESMOOTHER or the handle to
%      the existing singleton*.
%
%      PANDA_IMAGESMOOTHER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_IMAGESMOOTHER.M with the given input arguments.
%
%      PANDA_IMAGESMOOTHER('Property','Value',...) creates a new PANDA_IMAGESMOOTHER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_ImageSmoother_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_ImageSmoother_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_ImageSmoother

% Last Modified by GUIDE v2.5 22-Jul-2015 21:13:21

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_ImageSmoother_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_ImageSmoother_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_ImageSmoother is made visible.
function PANDA_ImageSmoother_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_ImageSmoother (see VARARGIN)

% Choose default command line output for PANDA_ImageSmoother
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_ImageSmoother wait for user response (see UIRESUME)
% uiwait(handles.PANDASmoothFigure);
global KernelSize;
global RawImages_ForSmooth;
global SmoothedImages;
KernelSize = 6;
set(handles.KernelSize_Edit, 'String', '6');
RawImages_ForSmooth = '';
SmoothedImages = '';
%
...
function varargout = PANDA_ImageSmoother_OutputFcn(hObject, eventdata, handles) 
...
global RawImages_ForSmooth;
global SmoothedImages;
if isempty(RawImages_ForSmooth)
    msgbox('Please input the raw images.');
elseif isempty(KernelSize)
    msgbox('Please input the kernel size.');
else
    for i = 1:length(RawImages_ForSmooth)
        cmd = ['fslmaths ' RawImages_ForSmooth{i} ' -s ' num2str(KernelSize) ' ' SmoothedImages{i}];
        disp(cmd);
        system(cmd);
    end
end
...
[~, RawImages_ForSmooth, Done] = PANDA_Select('img', RawImages_Button);
if Done == 1
    set(hObject, 'UserData', RawImages_ForSmooth);
    ResampledImages = '';
    if ~isempty(RawImages_ForSmooth)
        if isempty(KernelSize)
            set(handles.RawSmoothedImagesTable, 'data', RawImages_ForSmooth);
            RawSmoothedImagesTable(handles);
        else
            for i = 1:length(RawImages_ForSmooth)
                [InputImageParentFolder InputImageFileName Suffix] = fileparts(RawImages_ForSmooth{i});
                if strcmp(Suffix, '.gz')
                    SmoothedImages{i} = [InputImageParentFolder filesep InputImageFileName(1:end - 4) '_s' num2str(KernelSize) 'mm.nii.gz'];
                elseif strcmp(Suffix, '.nii')
                    SmoothedImages{i} = [InputImageParentFolder filesep InputImageFileName '_s' num2str(KernelSize) 'mm.nii.gz'];
                end
            end
            SmoothedImages = reshape(SmoothedImages, length(SmoothedImages), 1);
            RawSmoothedImages = [RawImages_ForSmooth SmoothedImages];
            set(handles.RawSmoothedImagesTable, 'data', RawSmoothedImages);
        end
    end
end
...
function KernelSize_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to KernelSize_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of KernelSize_Edit as text
%        str2double(get(hObject,'String')) returns contents of KernelSize_Edit as a double
global KernelSize;
global RawImages_ForSmooth;
global SmoothedImages;
KernelSizeString = get(hObject,'String');
if ~isempty(KernelSizeString)
    KernelSize = str2num(KernelSizeString);
    if isempty(KernelSize)
        msgbox('The kernel size you input is illegal.');
        set(hObject, 'String', '');
    else
        for i = 1:length(RawImages_ForSmooth)
            [InputImageParentFolder InputImageFileName Suffix] = fileparts(RawImages_ForSmooth{i});
...
function KernelSize_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to KernelSize_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
function RawSmoothedImagesTable(handles)
RawPathDestPath = get(handles.RawSmoothedImagesTable, 'data');
PositionFigure = get(handles.PANDASmoothFigure, 'Position');
...
set(handles.RawSmoothedImagesTable, 'ColumnWidth', WidthCell);
...
% --- Executes when PANDASmoothFigure is resized.
function PANDASmoothFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDASmoothFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    PositionFigure = get(handles.PANDASmoothFigure, 'Position');
