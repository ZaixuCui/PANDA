...
% GUI for merging 3D NIfTI to 4D NIfTI (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
% Last Modified by GUIDE v2.5 22-Jul-2015 21:18:42
...
% uiwait(handles.PANDAMergeNIfTIFigure);
% Set icon
SetIcon(handles);
%
...
[~, RawNIfTIList, Done] = PANDA_Select('img', RawNIfTIList_Button);
if Done == 1
...
% --- Executes when PANDAMergeNIfTIFigure is resized.
function PANDAMergeNIfTIFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDAMergeNIfTIFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    ResizeMergeNIfTITable(handles);
end

function ResizeMergeNIfTITable(handles)
RawNIfTIPath = get(handles.RawNIfTIPathTable, 'data');
PositionFigure = get(handles.PANDAMergeNIfTIFigure, 'Position');
WidthCell{1} = PositionFigure(3);
WidthCell{2} = 0;
if ~isempty(RawNIfTIPath)
    for i = 1:length(RawNIfTIPath)
        tmp_PANDA{i} = length(RawNIfTIPath{i}) * 8;
        tmp_PANDA{i} = tmp_PANDA{i} * PositionFigure(4) / 350;
    end
