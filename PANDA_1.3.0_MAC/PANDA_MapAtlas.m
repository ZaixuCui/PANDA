function varargout = PANDA_MapAtlas(varargin)
% GUI for mapping high resolution atlas to anatomical atlas (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_MAPATLAS MATLAB code for PANDA_MapAtlas.fig
%      PANDA_MAPATLAS, by itself, creates a new PANDA_MAPATLAS or raises the existing
%      singleton*.
%
%      H = PANDA_MAPATLAS returns the handle to a new PANDA_MAPATLAS or the handle to
%      the existing singleton*.
%
%      PANDA_MAPATLAS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_MAPATLAS.M with the given input arguments.
%
%      PANDA_MAPATLAS('Property','Value',...) creates a new PANDA_MAPATLAS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_MapAtlas_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_MapAtlas_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_MapAtlas

% Last Modified by GUIDE v2.5 06-Jul-2015 21:14:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_MapAtlas_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_MapAtlas_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_MapAtlas is made visible.
function PANDA_MapAtlas_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_MapAtlas (see VARARGIN)

% Choose default command line output for PANDA_MapAtlas
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_MapAtlas wait for user response (see UIRESUME)
% uiwait(handles.figure1);
global MapAtlas_OriginalAtlasPath;
global MapAtlas_AnatomicalAtlasPath;
global MapAtlas_ResultantFolder;
MapAtlas_OriginalAtlasPath = '';
MapAtlas_AnatomicalAtlasPath = '';
MapAtlas_ResultantFolder = '';
set(handles.OriginalAtlas_Edit, 'String', '');
set(handles.AnatomicalAtlas_Edit, 'String', '');
set(handles.ResultantFolder_Edit, 'String', '');
% Set icon
SetIcon(handles);
%
...
function varargout = PANDA_MapAtlas_OutputFcn(hObject, eventdata, handles) 
...
if isempty(MapAtlas_OriginalAtlasPath)
    msgbox('Please input the original atlas path!');
elseif isempty(MapAtlas_AnatomicalAtlasPath)
    msgbox('Please input the anatomical atlas path!');
elseif isempty(MapAtlas_ResultantFolder)
    msgbox('Please input the resultant folder path!');
else
    if ~exist(MapAtlas_ResultantFolder, 'dir')
        mkdir(MapAtlas_ResultantFolder);
    end
    TmpImg = [MapAtlas_ResultantFolder filesep 'tmp.nii'];
    % quantity of ROIs in original atlas
    PANDA_cmd = ['fslchfiletype NIFTI ' MapAtlas_OriginalAtlasPath ' ' TmpImg];
    system(PANDA_cmd);
    OriginAtlas_data = load_nii(TmpImg);
    OriginAtlas_Index = unique(OriginAtlas_data.img);
    OriginAtlas_Index = OriginAtlas_Index(2:end);
    OriginAtlas_ROIQuantity = length(OriginAtlas_Index);
    delete(TmpImg);
    fid = fopen([MapAtlas_ResultantFolder filesep 'MapAtlas.txt'], 'w');
    for i = 1:OriginAtlas_ROIQuantity

        disp(['Processing ROI ' num2str(OriginAtlas_Index(i))]);

        RegionI_OriginIndex = OriginAtlas_Index(i);
        fprintf(fid, '%04d', RegionI_OriginIndex);

        Region_I_mask = [MapAtlas_ResultantFolder filesep 'Region_' num2str(OriginAtlas_Index(i)) '_mask'];
        cmd = ['fslmaths ' MapAtlas_OriginalAtlasPath ' -thr ' num2str(i) ' -uthr ' num2str(OriginAtlas_Index(i)) ' -bin ' Region_I_mask];
        system(cmd);
        RegionI_In_Template = [MapAtlas_ResultantFolder filesep 'Region_' num2str(OriginAtlas_Index(i)) '_In_Template'];
        cmd = ['fslmaths ' MapAtlas_AnatomicalAtlasPath ' -mul ' Region_I_mask ' ' RegionI_In_Template];
        system(cmd);
        system(['fslchfiletype NIFTI ' RegionI_In_Template]);

        RegionI_data = load_nii([RegionI_In_Template '.nii']);
        RegionI_data_unique = unique(RegionI_data.img);

        RegionI_AnatomicalAtlas_Index = RegionI_data_unique(2:end);
        Voxel_Quantity = length(find(RegionI_data.img));
        for j = 1:length(RegionI_AnatomicalAtlas_Index)
            RegionI_AnatomicalAtlas_Portion(j) = length(find(RegionI_data.img == RegionI_AnatomicalAtlas_Index(j))) / Voxel_Quantity;
            fprintf(fid, ' %02d %0.3f ', RegionI_AnatomicalAtlas_Index(j), RegionI_AnatomicalAtlas_Portion(j));  
        end
        fprintf(fid, '\n');

        system(['rm -rf ' Region_I_mask '*']);
        system(['rm -rf ' RegionI_In_Template '*']);
    end
    disp(['Done!']);
end
...
% --- Executes on button press in OriginalAtlas_Button.
function OriginalAtlas_Button_Callback(hObject, eventdata, handles)
% hObject    handle to OriginalAtlas_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global MapAtlas_OriginalAtlasPath;
[Atlas_FolderName ParentFolder] = uigetfile({'*.nii.gz;*.nii;*.img','NIfTI-files (*.nii.gz,*.nii,*.img)'});
Atlas_FolderPath = [ParentFolder Atlas_FolderName];
if ~isnumeric(Atlas_FolderPath)
    set( handles.OriginalAtlas_Edit, 'string', Atlas_FolderPath );
    MapAtlas_OriginalAtlasPath = Atlas_FolderPath;
end



function OriginalAtlas_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to OriginalAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of OriginalAtlas_Edit as text
%        str2double(get(hObject,'String')) returns contents of OriginalAtlas_Edit as a double
global MapAtlas_OriginalAtlasPath;
set(hObject, 'String', MapAtlas_OriginalAtlasPath);


% --- Executes during object creation, after setting all properties.
function OriginalAtlas_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to OriginalAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
% --- Executes on button press in AnatomicalAtlas_Button.
function AnatomicalAtlas_Button_Callback(hObject, eventdata, handles)
% hObject    handle to AnatomicalAtlas_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global MapAtlas_AnatomicalAtlasPath;
[Atlas_FolderName ParentFolder] = uigetfile({'*.nii.gz;*.nii;*.img','NIfTI-files (*.nii.gz,*.nii,*.img)'});
Atlas_FolderPath = [ParentFolder Atlas_FolderName];
if ~isnumeric(Atlas_FolderPath)
    set( handles.AnatomicalAtlas_Edit, 'string', Atlas_FolderPath );
    MapAtlas_AnatomicalAtlasPath = Atlas_FolderPath;
end


function AnatomicalAtlas_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to AnatomicalAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of AnatomicalAtlas_Edit as text
%        str2double(get(hObject,'String')) returns contents of AnatomicalAtlas_Edit as a double
global MapAtlas_AnatomicalAtlasPath;
set(hObject, 'String', MapAtlas_AnatomicalAtlasPath);


% --- Executes during object creation, after setting all properties.
function AnatomicalAtlas_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to AnatomicalAtlas_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
if ~exist(ResultantFolder, 'dir')
    try
        ...
        MapAtlas_ResultantFolder = ResultantFolder;
    catch
...
% handles    structure with handles and user data (see GUIDATA)
global MapAtlas_ResultantFolder;
MapAtlas_ResultantFolder = uigetdir;
if MapAtlas_ResultantFolder
    if ~exist(MapAtlas_ResultantFolder, 'dir')
        mkdir(MapAtlas_ResultantFolder);
    end
    set( handles.ResultantFolder_Edit, 'String', MapAtlas_ResultantFolder);
end
