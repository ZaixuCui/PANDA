function varargout = PANDA_ROIExtract(varargin)
% GUI for extract ROIs from atlas (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%      Copyright(c) 2015
...
%      Last edited 
%--------------------------------------------------------------------------
% PANDA_ROIEXTRACT MATLAB code for PANDA_ROIExtract.fig
%      PANDA_ROIEXTRACT, by itself, creates a new PANDA_ROIEXTRACT or raises the existing
%      singleton*.
%
%      H = PANDA_ROIEXTRACT returns the handle to a new PANDA_ROIEXTRACT or the handle to
%      the existing singleton*.
%
%      PANDA_ROIEXTRACT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PANDA_ROIEXTRACT.M with the given input arguments.
%
%      PANDA_ROIEXTRACT('Property','Value',...) creates a new PANDA_ROIEXTRACT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PANDA_ROIExtract_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PANDA_ROIExtract_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help PANDA_ROIExtract

% Last Modified by GUIDE v2.5 22-Jul-2015 22:01:20

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PANDA_ROIExtract_OpeningFcn, ...
                   'gui_OutputFcn',  @PANDA_ROIExtract_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PANDA_ROIExtract is made visible.
function PANDA_ROIExtract_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PANDA_ROIExtract (see VARARGIN)

% Choose default command line output for PANDA_ROIExtract
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes PANDA_ROIExtract wait for user response (see UIRESUME)
% uiwait(handles.PANDAROIExtractFigure);
global ROIExtract_AtlasPath;
global ROIIndexList;
global ROIExtract_ResultantFolder;
ROIExtract_AtlasPath = '';
ROIIndexList = '';
ROIExtract_ResultantFolder = '';
set(handles.AtlasPath_Edit, 'String', '');
set(handles.ResultantFolder_Edit, 'String', '');
ROIIndexTable = cell(4,1);
set( handles.ROIIndexPathTable, 'data', ROIIndexTable);
if ~isempty(handles)
    ResizeROIIndexTxtTable(handles);
end
% Set icon
SetIcon(handles);
%
...
WidthCell{1} = 450;
WidthCell{2} = 0;
set( handles.ROIIndexPathTable, 'ColumnWidth', WidthCell);


% --- Outputs from this function are returned to the command line.
function varargout = PANDA_ROIExtract_OutputFcn(hObject, eventdata, handles) 
...
if ~isnumeric(Atlas_FolderPath)
    set( handles.AtlasPath_Edit, 'string', Atlas_FolderPath );
    ROIExtract_AtlasPath = Atlas_FolderPath;
end


function AtlasPath_Edit_Callback(hObject, eventdata, handles)
% hObject    handle to AtlasPath_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of AtlasPath_Edit as text
%        str2double(get(hObject,'String')) returns contents of AtlasPath_Edit as a double
global ROIExtract_AtlasPath;
set(hObject, 'String', ROIExtract_AtlasPath);


% --- Executes during object creation, after setting all properties.
function AtlasPath_Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to AtlasPath_Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
if ~exist(ResultantFolder, 'dir')
    try
        ...
        ROIExtract_ResultantFolder = ResultantFolder;
    catch
...
% handles    structure with handles and user data (see GUIDATA)
global ROIExtract_ResultantFolder;
ROIExtract_ResultantFolder = uigetdir;
if ROIExtract_ResultantFolder
    if ~exist(ROIExtract_ResultantFolder, 'dir')
        mkdir(ROIExtract_ResultantFolder);
    end
    set( handles.ResultantFolder_Edit, 'String', ROIExtract_ResultantFolder);
end
...
if isempty(ROIExtract_AtlasPath)
    msgbox('Please input the altas path!');
elseif isempty(ROIIndexList)
    msgbox('Please input the ROI index files (.txt format)!');
elseif isempty(ROIExtract_ResultantFolder)
    msgbox('Please input the resultant folder path!');
else
    for i = 1:length(ROIIndexList)
        [~, ~, Suffix] = fileparts(ROIIndexList{i});
        if ~strcmp(Suffix, '.txt')
            msgbox('The ROI index file should be .txt format!');
            return;
        end
    end
    clc;
    ...
    TmpImg = [ROIExtract_ResultantFolder filesep 'tmp.nii.gz']; 
    for i = 1:length(ROIIndexList)
        try
            ROIInfo = load(ROIIndexList{i}, '-ascii');
            [~, cols] = size(ROIInfo); 
            if cols >= 3
                msgbox(['The file ' ROIIndexList{i} ' should contain only two columns!']);
                return;
            end
        catch
            msgbox(['The txt file ' ROIIndexList{i} ' is illegal!']);
            return;
        end
        disp(['Processing ' ROIIndexList{i} ' ...']);
        [ParentFolder, FileName, ~] = fileparts(ROIIndexList{i});
        Resultant_File = [ROIExtract_ResultantFolder filesep FileName '_Extract.nii.gz'];
        ROIIndex = ROIInfo(:, 1);              
        if cols == 1 % does not contain weight
            PANDA_cmd = ['fslmaths ' ROIExtract_AtlasPath ' -thr ' num2str(ROIIndex(1)) ' -uthr ' num2str(ROIIndex(1)) ' ' Resultant_File];
            system(PANDA_cmd);
            for j = 2:length(ROIIndex)
                PANDA_cmd = ['fslmaths ' ROIExtract_AtlasPath ' -thr ' num2str(ROIIndex(j)) ' -uthr ' num2str(ROIIndex(j)) ' ' TmpImg];
                system(PANDA_cmd);
                PANDA_cmd = ['fslmaths ' Resultant_File ' -add ' TmpImg ' ' Resultant_File];
                system(PANDA_cmd);
            end
        elseif cols == 2 % contain weight
            ROIWeight = ROIInfo(:, 2);
            PANDA_cmd = ['fslmaths ' ROIExtract_AtlasPath ' -thr ' num2str(ROIIndex(1)) ' -uthr ' num2str(ROIIndex(1)) ' -bin -mul ' num2str(ROIWeight(1)) ' ' Resultant_File];
            system(PANDA_cmd);
            for j = 2:length(ROIIndex)
                PANDA_cmd = ['fslmaths ' ROIExtract_AtlasPath ' -thr ' num2str(ROIIndex(j)) ' -uthr ' num2str(ROIIndex(j)) ' -bin -mul ' num2str(ROIWeight(j)) ' ' TmpImg];
                system(PANDA_cmd);
                PANDA_cmd = ['fslmaths ' Resultant_File ' -add ' TmpImg ' ' Resultant_File];
                system(PANDA_cmd);
            end
        ...
        disp(['Done!']);
        disp(['Resultant file is ' Resultant_File]);
        fprintf('\n');
    end
    delete(TmpImg); 
end
...
% --- Executes on button press in ROIIndex_Button.
function ROIIndex_Button_Callback(hObject, eventdata, handles)
% hObject    handle to ROIIndex_Button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global ROIIndexList;
ROIIndexList_Button = get(hObject, 'UserData');
[~, ROIIndexList, Done] = PANDA_Select('file', ROIIndexList_Button);
if Done == 1
    set( hObject, 'UserData', ROIIndexList );
    if ~isempty(ROIIndexList)
        set( handles.ROIIndexPathTable, 'data', ROIIndexList);
    else
        ROIIndexTable = cell(4,1);
        set( handles.ROIIndexPathTable, 'data', ROIIndexTable);
    end
...
% --- Executes when PANDAROIExtractFigure is resized.
function PANDAROIExtractFigure_ResizeFcn(hObject, eventdata, handles)
% hObject    handle to PANDAROIExtractFigure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles)
    ResizeROIIndexTxtTable(handles);
...
function ResizeROIIndexTxtTable(handles)
ROIIndexTxt = get(handles.ROIIndexPathTable, 'data');
PositionFigure = get(handles.PANDAROIExtractFigure, 'Position');
WidthCell{1} = PositionFigure(3);
WidthCell{2} = 0;
if ~isempty(ROIIndexTxt)
    for i = 1:length(ROIIndexTxt)
        tmp_PANDA{i} = length(ROIIndexTxt{i}) * 8;
