...
% Last Modified by GUIDE v2.5 09-Mar-2014 21:49:32
...
%         set( handles.T1CroppingGapUnitText, 'Enable', 'off'); 
        ...
        if ~isfield( tracking_opt, 'T1Template')
            tracking_opt.T1Template = [PANDAPath filesep 'data' filesep 'Templates' filesep 'MNI152_T1_2mm_brain'];
        end
        set( handles.T1TemplateEdit, 'String', tracking_opt.T1Template);
    ...
    if ~isempty(SubjectIDArray)
        ...
        if ~isempty(TensorPrefixEdit)
            ...
            for i = 1:length(SubjectIDArray)
                FAPathCellMain{i} = [DestinationPath_Edit filesep num2str(SubjectIDArray(i),'%05.0f') filesep 'native_space' filesep num2str(SubjectIDArray(i),'%05.0f') '_' 'FA.nii.gz'];
            end
        end
...
if LockFlagMain | (~tracking_opt.DterminFiberTracking & ~tracking_opt.NetworkNode ...
    ...
    if tracking_opt.NetworkNode
        ...
        elseif tracking_opt.T1 
            ...
            elseif isempty(tracking_opt.T1Template)
    ...
    if TrackingOpt_ParametersChangeFlag
        ...
        if NetworkNodeInputFinish 
            ...
            if tracking_opt.T1
                ...
                info_quantity = info_quantity + 1;
                Check_info{info_quantity} = ['T1 template path = ' tracking_opt.T1Template];
                info_quantity = info_quantity + 1;
...
%     set( handles.ImageOrientationMenu, 'value', 1.0);
    ...
    tracking_opt.AngleThreshold = '45';
    set( handles.AngleThresholdEdit, 'String', tracking_opt.AngleThreshold);
...
global tracking_opt;
global TrackingOpt_ParametersChangeFlag;
T1Template_Previous = tracking_opt.T1Template;
T1TemplatePath = get( handles.T1TemplateEdit, 'string' );
tracking_opt.T1Template = T1TemplatePath;
if ~strcmp(T1Template_Previous, tracking_opt.T1Template)
    TrackingOpt_ParametersChangeFlag = 1;
end
