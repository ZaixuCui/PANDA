...
% GUI for Fiber Tracking (an independent component of software PANDA), by Zaixu Cui 
%-------------------------------------------------------------------------- 
%	Copyright(c) 2011
...
% Last Modified by GUIDE v2.5 05-May-2012 20:15:53
...
TrackingPipeline_opt.mode = 'batch';
set( handles.QsubOptionsEdit, 'String', '');
set( handles.QsubOptionsEdit, 'Enable', 'off');
TrackingPipeline_opt.path_logs = [pwd filesep 'logs'];
...
if ~isfield(trackingAlone_opt, 'DterminFiberTracking')
    ...
    trackingAlone_opt.AngleThreshold = '35';
    set( handles.AngleThresholdEdit, 'String', '35');
    ...
    trackingAlone_opt.MaskThresMin = 0.1;
    set( handles.MaskThresMinEdit, 'String', '0.1');    
...
if get(hObject, 'value') == 1
    if isempty(FAPathCellTracking)
        ...
        if strcmp(button,'Yes')
            ...
            set(hObject, 'value', 0);
            [a, FAPathCellTracking] = Select('img');
            if ~isempty(FAPathCellTracking)
        ...
        if trackingAlone_opt.PartitionOfSubjects
            ...
            trackingAlone_opt.PartitionTemplate = [PANDAPath filesep 'data' filesep 'atlases' filesep 'AAL' filesep 'AAL_116_2MM'];
...
global FAPathCellTracking;
global T1orPartitionOfSubjectsPathCellTracking;
[x,T1orPartitionOfSubjectsPathCellTracking] = Select('img');
...
if exist(LockFilePath, 'file') | strcmp(get(handles.LogPathEdit, 'Enable'), 'off')
    ...
    if trackingAlone_opt.DterminFiberTracking
        if isempty(NativePathCellTracking)
            msgbox('Please input native path for Deterministic Fiber Tracking !');
    ...
    if trackingAlone_opt.NetworkNode
        ...
        elseif trackingAlone_opt.T1
            if isempty(trackingAlone_opt.PartitionTemplate)
                msgbox('Please select atlas (standard space) !');
            end
        end
        if ~isempty(T1orPartitionOfSubjectsPathCellTracking) && ( trackingAlone_opt.PartitionOfSubjects | ...
                (trackingAlone_opt.T1 && ~isempty(trackingAlone_opt.PartitionTemplate)) )
    ...
    elseif DeterminFiberTrackingOptionsFinish & NetworkNodeOptionsFinish & PipelineOptionsFinish & ~LogPathPermissionDenied
        ...
        info{2} = 'If you are sure to run, please save the configuration first!';
        button = questdlg( info ,'Sure to Run ?','Yes','No','Yes' );
        switch button
            case 'Yes'
                % Save the configuration
                [ ParameterSaveFileName,ParameterSavePath ] = uiputfile( {'*.PANDA_Tracking', 'PANDA_Tracking-files (*.PANDA_Tracking)'},'Save Configuration' );
                ...
                if ParameterSavePath ~= 0
                    cmdString = [ 'save ' ParameterSaveFilePath ' TrackingPipeline_opt' ' trackingAlone_opt' ' PANDAPath'];
                    eval(cmdString);
                    ...
                    if exist( ParameterSaveFilePath, 'file' )
                        clc;
                        disp( 'The variable is saved sucessfully!' );
                    ...
                    if trackingAlone_opt.NetworkNode == 1
                        command = ['"' matlabroot filesep 'bin' filesep 'matlab" -nosplash -nodesktop -r "load(''' ParameterSaveFilePath ''',''-mat''); addpath(genpath(PANDAPath));'...
                            'pipeline=g_tracking_pipeline( NativePathCellTracking,trackingAlone_opt,TrackingPipeline_opt,FAPathCellTracking,T1orPartitionOfSubjectsPathCellTracking );exit"'...
                        ...
                        command = ['"' matlabroot filesep 'bin' filesep 'matlab" -nosplash -nodesktop -r "load(''' ParameterSaveFilePath ''',''-mat'');'...
                            'addpath(genpath(PANDAPath)); pipeline=g_tracking_pipeline( NativePathCellTracking,trackingAlone_opt,TrackingPipeline_opt );exit"'...
                            ' >"' TrackingPipeline_opt.path_logs filesep 'tracking_pipeline.loginfo" 2>&1'];
                    end
                    ...
                    system(['. ' TrackingPipelineShLocation ' &']);
                    fclose(fid);
                    %                 g_tracking_pipeline(TrackingPipeline_opt,trackingAlone_opt,NativePathCellTracking,FAPathCellTracking,T1orPartitionOfSubjectsPathCellTracking);
                    % Native Folder input
...
if strcmp(get(handles.LogPathEdit, 'Enable'), 'off')
    ...
    if ParameterSaveFileName ~= 0
        ...
        % Pipeline Options
        if strcmp(TrackingPipeline_opt.mode,'batch')
            set( handles.batchRadio, 'Value', 1);
        else
...
% --- Executes on selection change in NativeFolderListbox.
function NativeFolderListbox_Callback(hObject, eventdata, handles)
% hObject    handle to NativeFolderListbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns NativeFolderListbox contents as cell array
%        contents{get(hObject,'Value')} returns selected item from NativeFolderListbox
global NativePathSelectedTracking;
NativePathSelectedTracking = get(hObject, 'Value');


% --- Executes during object creation, after setting all properties.
function NativeFolderListbox_CreateFcn(hObject, eventdata, handles)
% hObject    handle to NativeFolderListbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
global NativePathCellTracking;
% global FiberTrackNativePathCellBefore;
[a, AddNativePathCell] = Select('dir');
NativePathCellTracking = [ NativePathCellTracking;AddNativePathCell ];
% FiberTrackNativePathCellBefore = NativePathCellTracking;
set(handles.NativeFolderListbox, 'Value', 1);
set(handles.NativeFolderListbox, 'String', NativePathCellTracking);


% --- Executes on button press in DeleteNativeFolderButton.
function DeleteNativeFolderButton_Callback(hObject, eventdata, handles)
% hObject    handle to DeleteNativeFolderButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global NativePathCellTracking;
global NativePathSelectedTracking;
NativePathCellTracking = get(handles.NativeFolderListbox, 'String');
if NativePathSelectedTracking 
    OldNativePathCell = NativePathCellTracking;
    NativePathCellTracking = '';
    j = 0;
    for i = 1:length(OldNativePathCell)
        if ~sum(ismember(NativePathSelectedTracking, i))
            j = j + 1;
            NativePathCellTracking{j} = OldNativePathCell{i};
        end
    end
    NativePathSelectedTracking = 0;
    NativePathCellTracking = reshape(NativePathCellTracking, length(NativePathCellTracking), 1);
    set(handles.NativeFolderListbox, 'Value', 1);
    set(handles.NativeFolderListbox, 'String', NativePathCellTracking);
end
...
%     end
    if isempty(NativePathCellTracking)
        ...
        if strcmp(BedpostxButton, 'Yes') 
            [a, NativePathCellTracking] = Select('dir');
            if ~isempty(NativePathCellTracking)
                set( handles.NativeFolderListbox, 'Value', 1);
                set(handles.NativeFolderListbox, 'String', NativePathCellTracking);
%                 ProbNativePathCellBefore = NativePathCellTracking;
    ...
    if ~isempty(NativePathCellTracking) && trackingAlone_opt.NetworkNode ...
            && ~isempty(T1orPartitionOfSubjectsPathCellTracking)
        [x, ProbabilisticTracking_opt, ProbabilisticOK] = Probabilistic_Opt;
        if ProbabilisticOK
...
%         set(hObject, 'Value', 0);
    ...
    if ~isempty(NativePathCellTracking) && trackingAlone_opt.NetworkNode ...
            && ~isempty(T1orPartitionOfSubjectsPathCellTracking)
        [x, Bedpostx_opt, ProbabilisticTracking_opt, BedpostxAndProbabilisticOK] = BedpostxAndProbabilistic_Opt;
...
if ~isempty(NativePathCellTracking)
    if strcmp(get( handles.ImageOrientationMenu, 'Enable' ), 'off')
...
TrackingPipeline_opt.path_logs = [LogPath filesep 'logs'];
...
switch get(hObject, 'tag')
    ...
    case 'qsubRadio'
        ...
        TrackingPipeline_opt.qsub_options = '-q all.q';
        set( handles.QsubOptionsEdit, 'Enable', 'on');
...
if ~isempty(handles)
    ...
    FontSizeNetworkConstructionUipanel = ceil(8 * PositionFigure(4) / 659);
    set( handles.NetworkConstructionUipanel, 'FontSize', FontSizeNetworkConstructionUipanel );
    FontSizePipelineOptionsUipanel = ceil(8 * PositionFigure(4) / 659);
