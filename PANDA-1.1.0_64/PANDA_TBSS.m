...
% Last Modified by GUIDE v2.5 06-May-2012 14:17:14
...
DataPathCellTBSS = '';
FADataPathCellTBSS = '';
[a, SelectFAPathCellTBSS] = Select('img');
if ~isempty(SelectFAPathCellTBSS)
    DataPathCellTBSS = '';
    FAPathCellTBSS = SelectFAPathCellTBSS;
    FADataPathCellTBSS = [FADataPathCellTBSS, FAPathCellTBSS];
...
if ~isempty(FAPathCellTBSS)
    [a, SelectDataPathCell] = Select('img');
    ...
    elseif isempty(TBSSPipeline_opt.max_queued)
        ...
        if ~LogPathPermissionDenied
            ...
            if strcmp(button, 'Yes')
                ...
                [ ParameterSaveFileName,ParameterSavePath ] = uiputfile( {'*.PANDA_TBSS', 'TBSS-files (*.PANDA_TBSS)'},'Save Configuration' );
                ...
                if ParameterSavePath ~= 0
                    ...
                    if exist( ParameterSaveFilePath, 'file' )
                        ...
                        fclose(fid);
    %                     g_TBSS_pipeline( FAPathCellTBSS, DataPathCellTBSS, ThresholdTBSS, TBSSResultPath, TBSSPipeline_opt );
...
if strcmp(get(handles.FAPathButton, 'Enable'), 'off')
    ...
    if ParameterSaveFileName ~= 0
        ...
        if ~isempty(TBSSResultPath) && exist(LockFilePath, 'file')
            ...
            set( handles.ThresholdEdit, 'string', num2str(ThresholdTBSS) );
            set( handles.TBSSResultPathEdit, 'string', TBSSResultPath );
            if strcmp(TBSSPipeline_opt.mode,'batch')
                set( handles.batchRadio, 'Value', 1 );
            else
...
if exist( StatusFilePath, 'file' )
    ...
    try    
        ...
        for i = 1:cols  % cols is the quantity of data type to be projected to the skeleton
            JobName = [JobName, [ 'skeleton_Data', num2str(i) ]];
        end
...
if ~isempty(DataPathCellTBSS) 
    [rows, cols] = size(DataPathCellTBSS);
    for i = 1:ColumnQuantity
        WidthCell{i} = PositionFigure(3) / (cols + 1);
    end
elseif ~isempty(FAPathCellTBSS)
    for i = 1:ColumnQuantity
        WidthCell{i} = PositionFigure(3);
    end
else
    for i = 1:ColumnQuantity
        WidthCell{i} = PositionFigure(3) / 4;
    end
end
